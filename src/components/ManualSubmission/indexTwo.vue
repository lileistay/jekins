<template>
  <div>
  <sg-dialog
    :dialog="showManualDialog"
    :inner-dialog="innerDialog"
    :inner-fullscreen="true"
    title="客户手工上报回访计划"
    inner-title="上报回访"
    inner-title-icon="el-icon-s-unfold"
    @innerHandleClose="closeDialog('closeInnerDialog')"
    @handleClose="closeDialog('closeShowManualDialog')"
  >
    <div slot="out">
      <div class="dialogTop">
        回访客户数量:{{ selectionData.length }};&nbsp;&nbsp;&nbsp;&nbsp;下单人:{{ $store.getters.user.nickName }}
        <el-button @click="selectAll" type="primary" size="mini" style="margin-left: 30px;" v-if="flagnew">选择全部人员</el-button>
      </div>
            ...
            <el-row>
              <el-col :span="12">
                <el-form-item label="是否一次性回访" prop="isOneReturnVisit">
                  <el-select
                    v-model="returnVisitRecordList[0].isOneReturnVisit"
                    class="full-width"
                    placeholder="请选择"
                  >
                    <el-option
              ...
              <el-col :span="12">
                <el-form-item label="计划日期" :prop="valueqj==true?'planReturnTimes':'planReturnTime'">
                  <el-date-picker
                    v-model="returnVisitRecordList[0].planReturnTime"
                    class="full-width"
                    type="datetime"
                    :clearable="false"
              ...
              <el-col :span="12">
                <el-form-item label="任务等级" prop="grade">
                  <!-- @change="change" -->
                  <el-select
                    v-model="returnVisitRecordList[0].grade"
                    class="full-width"
                    placeholder="请选择"
                  >
                    <el-option
              ...
              <el-col :span="12">
                <el-form-item label="回访阶段" prop="returnStage">
                  <el-select
                    v-model="returnVisitRecordList[0].returnStage"
                    class="full-width"
                    placeholder="请选择"
                  >
                    <el-option
              ...
              <el-col :span="12">
                <el-form-item label="任务关键字">
                  <el-input
                    v-model="returnVisitRecordList[0].returnVisitKeyword"
                    class="full-width"
                    placeholder="请输入"
                  />
                </el-form-item>
              ...
              <!--     <el-select-->
              <!--       v-model="returnVisitRecordList[0].aestheticConsultant "-->
              <!--       class="full-width"-->
              <!--       placeholder="请选择"-->
              <!--     >-->
              <!--       <el-option-->
              <!--         v-for="(items, index) in sysUserList"-->
              <!--         :key="index"-->
              <!--         :label="items.label"-->
              <!--         :value="items.value"-->
              <!--       />-->
              ...
              <!--     <el-select-->
              <!--       v-model="returnVisitRecordList[0].customerRepresentative "-->
              <!--       class="full-width"-->
              <!--       placeholder="请选择"-->
              <!--     >-->
              <!--       <el-option-->
              ...
              <el-col :span="24">
                <el-form-item label="回访目的">
                  <el-input
                    v-model="returnVisitRecordList[0].returnVisit"
                    class="full-width"
                    :rows="2"
                    resize="none"
                    type="textarea"
            ...
            <el-row>
              <el-col :span="12">
                <el-form-item label="指定执行人">
                  <el-checkbox
                    v-model="returnVisitRecordList[0].appointPeople"
                    @change="Onechange"
                  />
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item label="选择执行人">
                  <el-select
                    v-model="returnVisitRecordList[0].planReturnUser"
                    filterable
                    remote
                    reserve-keyword
                    placeholder="请输入"
                    :disabled="!returnVisitRecordList[0].appointPeople"
              ...
              <el-col :span="12">
                <el-form-item label="执行类型">
                  <el-select
                    v-model="returnVisitRecordList[0].returnUserType"
                    :disabled="returnVisitRecordList[0].appointPeople"
                    placeholder="请选择"
                    class="full-width"
                  >
                    <el-option
                      v-for="dict in dict.type['execution_type']"
                      :key="dict.value"
                      :label="dict.label"
                      :value="dict.value"
                    />
              ...
              <el-col :span="12">
                ...
                >
                  <el-select
                    v-model="returnVisitRecordList[0].backupExecutor"
                    filterable
                    remote
                    reserve-keyword
                    placeholder="请输入关键词"
      ...
      <el-button
        size="mini"
        type="primary"
        @click.stop="saveManual('manualForm')"
      >保存</el-button>
    </div>
    <div slot="in">
      <el-card class="visitBox">
        <ul>
          ...
          <!-- <el-scrollbar> -->
            ...
            <el-card
              v-for="(item,index) in returnVisitRecordList"
              :key="index"
              class="visitBox innerVisitBox"
            >
              <ul>
                <li>计划{{ index + 1 }}<a>（若不能确定回访指定执行人,可选择填写执行人类型,系统自动匹配 ）</a>
                  <el-link
                    ...
                    style="float: right"
                    size="mini"
                    @click.stop="deleteManualItem(index)"
                  >删除
                  </el-link>
                </li>
                    ...
                    >
                      <el-row>
                        <el-col :span="12">
                          <el-form-item
                            label="是否一次性回访"
                            prop="isOneReturnVisit"
                          >
                            <el-select
                              v-model="item.isOneReturnVisit"
                              style="width: 100%;"
                              size="mini"
                              placeholder="请选择"
                            >
                        ...
                        <el-col :span="12">
                          <el-form-item label="关键字">
                            <el-input
                              v-model="item.returnVisitKeyword"
                              class="full-width"
                              placeholder="请输入"
                            />
                          </el-form-item>
                    ...
                    >
                      <el-row>
                        <el-col :span="10">
                          <el-form-item label="指定执行人">
                            <el-checkbox
                              v-model="item.appointPeople"
                              @change="change(e,item)"
                            />
                          </el-form-item>
                        </el-col>
                        <el-col :span="14">
                          <el-form-item label="选择执行人">
                            <el-select
                              v-model="item.planReturnUser"
                              filterable
                              remote
                              reserve-keyword
                              placeholder="请输入"
                              :loading="!item.appointPeople"
                              :remote-method="querySearchAsync"
                              :disabled="loading"
                            >
                              <el-option
                                v-for="items in sysUserList"
                                :key="items.value"
                                :label="items.label"
                                :value="items.value"
                              />
...
export default {
  ...
  methods: {
    ...
    saveManual(form, isAddIndex) {
      ...
      this.returnVisitRecordList[0].aestheticConsultant = this.action.aestheticConsultant
      this.returnVisitRecordList[0].customerRepresentative = this.action.customerRepresentative
