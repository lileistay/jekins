...
export default {
  ...
  mounted() {
    this.queryData()
    this.queryName=this.$route.query.item.queryName
  },
  watch:{
    itemName(val,val1){
      this.queryName=val
  ...
  methods: {
    ...
    rowspan(row) {
      ...
      this.mergingRows=[]
      for (let i = 0; i < this.listData.length; i++) { // tabledata 表格数据源
        if (i === 0) {
          this.mergingRows.push(1)
          this.mergingPos = 0
        } else {
          if (this.listData[i].level === this.listData[i - 1].level) { //哪些数据是要合并的 合并的条件是什么
