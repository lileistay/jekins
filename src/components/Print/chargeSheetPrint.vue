<template>
  <div class="print">
    <div style="width: 100%;height: 100%" v-for="item in codeArr">
      <div class="title">{{print.billType}}</div>
      <div class="header">
        <span>姓名：{{ print.customName }}</span>
        <span>电话：{{ print.customPhone }}</span>
        <span>收费单号：{{ print.orderNumber }}</span>
        <span>客户卡号：{{ printcustomCard }}</span>
      </div>
      <div class="headers" style="font-size: 14px;margin-top: 10px">
<!--        <span style="margin-left: 85px">收费单类型：{{ print.billType }}</span>-->
        <span style="margin-left: 105px">备注：{{ print.billingRemarks }}</span>
      </div>
      <div class="table">
        <el-table
