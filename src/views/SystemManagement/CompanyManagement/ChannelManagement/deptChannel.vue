          ...
          <div class="dept-table">
            ...
            >
              <div slot="searchBar" style="display: flex;align-items: center">
                ...
                <el-select v-model="pagination.typeId" style="width: 200px;" size="mini" clearable filterable class="header-search-item" placeholder="请选择渠道类型">
                  <el-option
              ...
              <div slot="header">
                <el-button v-has-permi="['share:channelDeptTypeRelation:add']" size="mini" plain type="primary" icon="el-icon-plus" :disabled="addBan" @click="handleAdd">添加</el-button>
  ...
  methods: {
    ...
    handleNodeClick(data,item) {
      ...
      if (item.children!==undefined){
        ...
        this.getData()
        this.$api.modules.channel.deptChannelList({deptId:this.pagination.deptId}).then(res => {
          if (res) {
    ...
    // 删除操作
    handleDelete(e) {
      const deptChannel = e.deptChannel || this.ids
      this.$confirm('是否确认删除编号为' + deptChannel + '的数据？', '警告', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async() => {
        await this.$api.modules.channel.deptChannelOperate({}, 'delete', deptChannel).then(res => {
          if (res) {
            this.getData()
    ...
    getData: debounce(function(isSearch) {
      ...
      this.$api.modules.channel.deptChannelList(this.pagination).then(res => {
        if (res) {
