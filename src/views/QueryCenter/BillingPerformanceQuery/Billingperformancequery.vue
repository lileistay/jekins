          ...
          <div slot="searchBar" class="header-float">
            ...
            <div style="font-size: 12px">
              ...
              <p style="margin-top: 7px">
                <span>实付总额: {{ form.Payment }}</span>&nbsp;&nbsp;
                <span>正常收费单: {{ form.normalBill }}</span>&nbsp;&nbsp;
                <span>交预约金: {{ form.deposit }}</span>&nbsp;&nbsp;
                <span>交储值: {{ form.depositValue }}</span>&nbsp;&nbsp;
                <span>交住院押金: {{ form.hospitalization }}</span>&nbsp;&nbsp;
                <span>辅助治疗: {{ form.adjuvant }}</span>&nbsp;&nbsp;
                <span>检验: {{ form.inspection }}</span>&nbsp;&nbsp;
                <span>住院结算单: {{ form.statement }}</span>&nbsp;&nbsp;
                <span>交储值卡: {{ form.cardValue }}</span>&nbsp;&nbsp;
                <span>还款单: {{ form.repayment }}</span>&nbsp;&nbsp;
              </p>
              <p style="margin-top: 7px">
                <span>退款总额: {{ form.refundMoney }}</span>&nbsp;&nbsp;
                <span>退款单: {{ form.refund }}</span>&nbsp;&nbsp;
                <span>退预约金: {{ form.refundAdvance }}</span>&nbsp;&nbsp;
                <span>退储值: {{ form.refundValue }}</span>&nbsp;&nbsp;
                <span>退住院押金: {{ form.refundHospitalization }}</span>&nbsp;&nbsp;
                <span>退辅助治疗: {{ form.refundAdjuvant }}</span>&nbsp;&nbsp;
                <span>退检验: {{ form.refundInspection }}</span>&nbsp;&nbsp;
                <span>退住院结算单: {{ form.refundStatement }}</span>&nbsp;&nbsp;
                <span>退储值卡: {{ form.refundCardValue }}</span>&nbsp;&nbsp;
              </p>
            </div>
          </div>
