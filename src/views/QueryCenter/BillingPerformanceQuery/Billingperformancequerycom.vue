  ...
  <div class="container">
    <slideSearch :options="options">
      <div slot="btnBar">
        ...
        <el-button size="mini" v-has-permi="['confinement:queryCenter:exportBilling']" icon="el-icon-upload2"
      ...
      <slot>
        ...
        <SgTable :table-data="tableData" index="序号" size="mini" border :columns="columns" :pagination="pagination"
          ...
          <div slot="searchBar" class="header-float">
            ...
            <div style="margin-left:58%;position: absolute;width: 700px;">
              <ul>
                <li>
                  开单业绩:
                  <span>{{ form.transactionMoney }}</span>&nbsp;&nbsp;&nbsp;
                  <span>[ 总人数：{{ form.customerToal }} 新客首次：{{ form.newCustomerFitst }} 新客二次：{{ form.newCustomerSecond }}
                    新客：{{ form.newCustomer }} 老客：{{ form.regularCustomer }} 返款后业绩：0.00 ]</span>
                </li>
                <div style="width: 100%;height: 34px;font-size: 12px;display: flex;">
                  <div>
                    <p>实付总额:</p>
                    <p class="pay">{{ form.Payment.toFixed(0) | numberToCurrency }}</p>
                  </div>
                  <div style="margin-left: 20px;">
                    <p>正常收费单:</p>
                    <p class="pay">{{ form.normalBill == 0 ? form.normalBill : form.normalBill.toFixed(0) | numberToCurrency }}
                    </p>
                  </div>
                  <div style="margin-left: 20px;">
                    <p>交预约金:</p>
                    <p class="pay">{{ form.deposit == 0 ? form.deposit : form.deposit.toFixed(0) | numberToCurrency }}</p>
                  </div>
                  <div style="margin-left: 20px;">
                    <p>交储值:</p>
                    <p class="pay">{{ form.depositValue == 0 ? form.depositValue : form.depositValue.toFixed(0) | numberToCurrency
                    }}</p>
                  </div>
                  <div style="margin-left: 20px;">
                    <p>交住院押金:</p>
                    <p class="pay">{{
                      form.hospitalization == 0 ? form.hospitalization : form.hospitalization.toFixed(0) | numberToCurrency }}</p>
                  </div>
                  <div style="margin-left: 20px;">
                    <p>辅助治疗:</p>
                    <p class="pay">{{ form.nadjuvant == null ? form.nadjuvant : form.nadjuvant.toFixed(0) | numberToCurrency }}
                    </p>
                  </div>
                  <div style="margin-left: 20px;">
                    <p>检验:</p>
                    <p class="pay">{{ form.inspection == 0 ? form.inspection : form.inspection.toFixed(0) | numberToCurrency }}
                    </p>
                  </div>
                  <div style="margin-left: 20px;">
                    <p>住院结算单:</p>
                    <p class="pay">{{ form.statement == 0 ? form.statement : form.statement.toFixed(0) | numberToCurrency }}</p>
                  </div>
                  <div style="margin-left: 20px;">
                    <p>交储值卡:</p>
                    <p class="pay">{{ form.cardValue == 0 ? form.cardValue : form.cardValue.toFixed(0) | numberToCurrency }}</p>
                  </div>
                  <div style="margin-left: 20px;">
                    <p>还款单:</p>
                    <p class="pay">{{ form.repayment == 0 ? form.repayment : form.repayment.toFixed(0) | numberToCurrency }}</p>
                  </div>
                </div>
                <!-- //下一级 -->
                <div style="width: 100%;height: 34px;font-size: 12px;display: flex;">
                  <div>
                    <p>退款总额:</p>
                    <p class="pay">{{ form.refundMoney.toFixed(0) | numberToCurrency }}</p>
                  </div>
                  <div style="margin-left: 20px;">
                    <p>退款单:</p>
                    <p class="pay">{{ form.refund == 0 ? form.refund : form.refund.toFixed(0) | numberToCurrency }}</p>
                  </div>
                  <div style="margin-left: 20px;">
                    <p>退预约金:</p>
                    <p class="pay">{{
                      form.refundAdvance == 0 ? form.refundAdvance : form.refundAdvance.toFixed(0) | numberToCurrency }}</p>
                  </div>
                  <div style="margin-left: 20px;">
                    <p>退储值:</p>
                    <p class="pay">{{ form.refundValue == 0 ? form.refundValue : form.refundValue.toFixed(0) | numberToCurrency }}
                    </p>
                  </div>
                  <div style="margin-left: 20px;">
                    <p>退住院押金:</p>
                    <p class="pay">{{
                      form.refundHospitalization == 0 ? form.refundHospitalization : form.refundHospitalization.toFixed(0) | numberToCurrency
                    }}</p>
                  </div>
                  <div style="margin-left: 20px;">
                    <p>退辅助治疗:</p>
                    <p class="pay">{{
                      form.refundAdjuvant == null ? form.refundAdjuvant : form.refundAdjuvant.toFixed(0) | numberToCurrency }}</p>
                  </div>
                  <div style="margin-left: 20px;">
                    <p>退检验:</p>
                    <p class="pay">{{
                      form.refundInspection == 0 ? form.refundInspection : form.refundInspection.toFixed(0) | numberToCurrency }}
                    </p>
                  </div>
                  <div style="margin-left: 20px;">
                    <p>退住院结算单:</p>
                    <p class="pay">{{
                      form.refundStatement == 0 ? form.refundStatement : form.refundStatement.toFixed(0) | numberToCurrency }}</p>
                  </div>
                  <div style="margin-left: 20px;">
                    <p>退储值卡:</p>
                    <p class="pay">{{
                      form.refundCardValue == 0 ? form.refundCardValue : form.refundCardValue.toFixed(0) | numberToCurrency }}</p>
                  </div>
                </div>
                <!-- <li>
                  <span>实付总额:</span>&nbsp;
                ...
                <li>
                  <span class="pay">{{ form.Payment.toFixed(0)|numberToCurrency }}</span>
          ...
          <template #custom="data">
            ...
            <div v-else-if="data.props === 'customName'">
              <a style="color: #5f94de;cursor:pointer" @click="customerclick(data.custom.id, data.custom)">{{
                data.custom.customName }} </a><span style="font-size: 14px">{{ data.custom.customSex === '0' ? '♀' : '♂' }}
...
export default {
  ...
  name: 'Billingperformancequerycom',
  components: {
    slideSearch,
    SgTable
  },
    ...
    getData: debounce(async function (isSearch, ispage) {
      ...
      var form = {
        ...
        params: {
          ...
          projectTypeId: data.typeThreeIds,
