            ...
            <el-row :gutter="20">
              <el-col :span="6">
                <el-form-item label="现场咨询:" prop="aestheticConsultant">
                  <el-select v-model="formInline.aestheticConsultant" placeholder="请选择" clearable filterable>
                    <el-option
              ...
              <el-col :span="6">
                <el-form-item label="洗浴师:" prop="chanKangShi" style="margin-left: 80px;">
                  <el-select v-model="formInline.chanKangShi" clearable filterable>
                    <el-option
                      v-for="item in therapeutist"
                      :key="item.value"
                      :label="item.label"
                      :value="item.value"
                    />
  ...
  methods: {
    ...
    discountAmount(index, value) {
      if (value !== undefined) {
        ...
        this.tableData.forEach(ite => {
          if (parseFloat(ite.discountAmount) > 0) {
            // ('parseFloat(ite.discountAmount) > 0', parseFloat(ite.discountAmount) > 0)
            total = math.add(total, ite.discountAmount).toFixed(2)
          }
        })
        this.accountsPayable = total
