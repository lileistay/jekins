          ...
          >
            <el-row :gutter="20">
              ...
              <el-col :span="6">
                ...
                <el-form-item label="治疗时长(分)：" prop="treatmentLength">
                  <el-input-number
                    v-model="detail.treatmentLength"
                    :precision="0"
                    :min="1"
                    style="width: 165px;"
                  />
            ...
            <el-row>
              <el-col :span="6">
                <!-- 科室类型为4是整形类型，有麻醉师 -->
                <el-form-item
                  v-if="detail.departmentType === '4'"
                  label="麻醉师："
                  prop="anesthesiologist"
                >
                  <el-select
                    v-model="detail.anesthesiologist"
                    :filterable="true"
                    placeholder="请选择麻醉师"
                  >
                    <el-option
              ...
              <el-col :span="6">
                ...
                >
                  <el-select
                    v-model="detail.anesthesiaMethod"
                    :filterable="true"
                    placeholder="请选择麻醉方式"
                  >
                    <el-option
...
export default {
  ...
  methods: {
    ...
    init() {
      // 得到上一个页面传递的多个发起治疗的数据
      this.details = this.$store.getters.directTreatmentSelectionData
      // 左侧的客户信息使用数组第一个的数据
      ...
      this.details.forEach(detail => {
        // 默认本次治疗次数为1
        this.$set(detail, 'thisScribingNum', 1)
        // 循环赋值本次为第几次治疗
        this.$set(detail, 'numberTime', math.add(detail.treatmentNum, 1))
        // 实际执行人默认是医生
        this.$set(detail, 'deviceId', undefined)
        this.$set(detail, 'actualExecutorUser', detail.doctorId ? detail.doctorId : undefined)
        ...
        this.$set(detail, 'terraceType', undefined) // 排台类型
        this.$set(detail, 'treatmentLength', undefined)
        this.$set(detail, 'anesthesiaMethod', undefined)
        this.$set(detail, 'anesthesiologist', undefined)
      ...
      Promise.all(promiseList).then(() => {
        ...
        this.details.forEach(detail => {
          const deductionRecord = {
            ...
            doctorId: detail.actualExecutorUser, // 医生
