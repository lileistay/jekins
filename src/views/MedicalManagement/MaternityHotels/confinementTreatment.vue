  ...
  <div>
    ...
    >
        ...
        <!-- <el-form ref="form" :model="form" size="mini" label-width="150px" :rules="rules">
          <div style="display: flex;flex-wrap: wrap;">
            ...
            <el-form-item label="客户卡号：">
              <el-input v-model="form.customCardNumber" style="width: 100%;" :clearable="true" />
            </el-form-item>
            <el-form-item label="实际执行人：" prop="actualExecutorUser">
              <el-select  v-model="form.actualExecutorUser" :filter-method="actualFilter" :clearable="true" :filterable="true" style="width: 100%;">
                <el-option
            ...
            <el-form-item label="现场咨询：">
              <el-select v-model="form.aestheticConsultant" :filter-method="aestheFilter" :clearable="true" :filterable="true" style="width: 100%;">
                <el-option
                  v-for="(item, index) in aestheticConsultantList"
                  :key="index"
                  :label="item.label"
                  :value="item.value"
                />
            ...
            <el-form-item label="回访类型：" prop="returnTypeId">
              <el-select v-model="form.returnTypeId" :clearable="true" :filterable="true" style="width: 100%;">
                <el-option
            ...
            <el-form-item label="电话：">
              <el-input v-model.number="form.customPhone" maxlength="11" onkeyup = "value=value.replace(/[^\d]/g,'')" :clearable="true" />
            </el-form-item>
            <el-form-item label="责任月嫂：" prop="nursing">
              <el-select v-model="form.nursing" :filter-method="nursingFilter" :clearable="true" :filterable="true" style="width: 100%;">
                <el-option
            ...
            <el-form-item label="产康师/洗浴师：">
              <el-select v-model="form.doctorId" :filter-method="doctorFilter" :clearable="true" :filterable="true" style="width: 100%;">
                <el-option
                  v-for="(item, index) in doctorIdList"
                  :key="index"
                  :label="item.label"
                  :value="item.value"
                />
            ...
            <el-form-item label="责任护士：" prop="circulatingNurse">
              <el-select v-model="form.circulatingNurse" :filter-method="circulatingFilter" :clearable="true" :filterable="true" style="width: 100%;">
                <el-option
                  v-for="(item, index) in circulatingNurseList"
                  :key="index"
                  :label="item.label"
                  :value="item.value"
                />
            ...
            <el-form-item label="治疗状态：" prop="state">
              <el-select v-model="form.state" :clearable="true" :filterable="true" style="width: 100%;">
                <el-option
                  v-for="item in dict.type.treatment_type"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                />
      ...
      <!-- <div slot="header" class="header" >
          ...
          <el-button size="mini" class="headerBarBtn" type="primary" icon="el-icon-edit-outline" @click="customerDemand">客户需求</el-button>
          <el-button size="mini" class="headerBarBtn" type="primary" icon="el-icon-plus" @click="openManualDialog">手工上报</el-button>
          <el-button size="mini"  class="headerBarBtn" :plain="true" type="danger" icon="el-icon-close" @click="cancel">取消治疗</el-button>
          <el-button size="mini" class="headerBarBtn" :plain="true" type="primary" icon="el-icon-plus" @click="initiateTreatment">发起治疗</el-button>
      </div> -->
      <div slot="header" style="width: 100%;">
        <span style="font-size: 14px;"><i class="el-icon-s-data" style="color: #5f94de" />列表清单</span>
          <span style="font-size: 14px;"><span class="label" style="background-color: #ffffff;"></span><span style="margin-left: 5px;">已安排</span></span>
          <span style="font-size: 14px;"><span class="label" style="background-color: #c6f9c3;"></span><span style="margin-left: 5px;">已治疗</span></span>
          <span style="font-size: 14px;"><span class="label" style="background-color: #ffff00;"></span><span style="margin-left: 5px;">已取消</span></span>
        <el-button size="mini" type="primary" plain
                   class="headerBarBtn" @click="initiateTreatmentcode">扫码签名
        </el-button>
            <el-button v-hasPermi="['scribing:deductionRecord:selectTreatment:edit']" size="mini" type="info" plain
                       class="headerBarBtn" @click="customerDemand">客户需求
            </el-button>
            <el-button v-hasPermi="['scribing:deductionRecord:selectTreatment:customerCmtedit']" size="mini" type="primary" plain
                       class="headerBarBtn" @click="openManualDialog">手工上报
            </el-button>
            <el-button v-has-permi="['scribing:deductionRecord:selectTreatment:modification']" size="mini" type="success" plain
                       class="headerBarBtn" @click="cancel">取消治疗
            </el-button>
            <el-button v-hasPermi="['scribing:deductionRecord:selectTreatment:Handmade']" size="mini" type="primary" plain
                       class="headerBarBtn" @click="initiateTreatment">发起治疗
            </el-button>
...
export default {
  ...
  data() {
    return {
      ...
      nursingList: [], // 月嫂
      nursing: [],
      circulatingNurseList: [], // 责任护士
      circulatingNurse: [],
      aestheticConsultantList: [], // 巡回护士
      aestheticConsultant: [],
      doctorIdList: [], // 产康
      doctorList: [],
      tableData: [],
      ...
      rules: {
        // doctorId: [
        //   { required: true, message: '请选择产康师/洗浴师', trigger: 'change' }
        // ]
      },
      pagination: {
        pageSize: 20, // 每页显示的条数
  ...
  mounted(){
    document.querySelector('.sg-table-bar').style.height='160px';
  },
  watch: {
    deptId(val) {
      this.deptId = val
  ...
  created() {
    ...
    this.getData()
    setTimeout(()=>{
    this.options.state.options=this.dict.type.treatment_type
    },500)
    const this_ = this
    // 点击回车后事件
  ...
  methods: {
    ...
    nursingFilter(val) {
      if (val) {
        this.nursingList = this.nursing.filter((item) => {
          if (item.nickName.toUpperCase().indexOf(val.toUpperCase()) !== -1) {
            return true
          }
          return PinyinMatch.match(item.nickName, val)
        ...
        this.nursingList = this.nursing
      }
    },
    circulatingFilter(val) {
      if (val) {
        this.circulatingNurseList = this.circulatingNurse.filter((item) => {
          if (item.nickName.toUpperCase().indexOf(val.toUpperCase()) !== -1) {
            return true
          }
          return PinyinMatch.match(item.nickName, val)
        ...
        this.circulatingNurseList = this.circulatingNurse
      }
    },
    aestheFilter(val) {
      if (val) {
        this.aestheticConsultantList = this.aestheticConsultant.filter((item) => {
          if (item.nickName.toUpperCase().indexOf(val.toUpperCase()) !== -1) {
            return true
          }
          return PinyinMatch.match(item.nickName, val)
        ...
        this.aestheticConsultantList = this.aestheticConsultant
      }
    },
    doctorFilter(val) {
      if (val) {
        this.doctorIdList = this.doctorList.filter((item) => {
          if (item.nickName.toUpperCase().indexOf(val.toUpperCase()) !== -1) {
            return true
          }
          return PinyinMatch.match(item.nickName, val)
        ...
        this.doctorIdList = this.doctorList
      }
    },
    expandChange(row) {
      if (row[2].length) {
    ...
    */
    getData: debounce(function(isSearch) {
      ...
      //     // let spiStr = ''
      //     // if (this.form.customPhone.includes(' ')) {
      //     //   const spi = this.form.customPhone.split(' ')
      //     //   spi.forEach(value => {
      //     //     spiStr += value
      //     //   })
      //     // }
      //     // this.form.customPhone = spiStr
        ...
        if (res) {
          ...
          res.data.forEach(item => {
            if (item.postCode === 'zerennurse') {
              ...
              this.circulatingNurse = this.circulatingNurseList;
          ...
          res.data.forEach(item => {
            if (item.postCode === 'xczxs') {
              ...
              this.aestheticConsultant = this.aestheticConsultantList;
          ...
          res.data.forEach(item => {
            if (item.postCode === 'chks') {
              this.doctorIdList.push(item)
              this.doctorList = this.doctorIdList;
