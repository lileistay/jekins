...
import { beforeDayTime, todayTime,newgetData } from '@/utils/time'
...
export default {
    ...
    data() {
        return {
            ...
            upload: {
                ...
                updateSupport: 0,
                // 上传的地址
                url: '/api/confinement/customerCmt/importData'
            },
            tableDatas: [],
            submitLoading: false,
            // 多选数据
    ...
    //   }).then(res=>{
    //         this.$store.dispatch('setTriageSelectionData', res.rows)
    ...
    //   })
        ...
        getData: debounce(function (isSearch) {
            ...
            let date=data.endstartTime;
            let deta=data.endstartOrderTime;
            delete data.endstartTime;
            delete data.endstartOrderTime
            data.endTime=date;
            ...
            this.$set(data, 'deptId', this.deptId)
            this.showDataDialog = true
            this.$api.confinementRoom.subsidiaryIddecl(data).then(res => {
                if (res) {
                    this.tableData = res.rows
