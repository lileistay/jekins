<template>
  ...
  <div class="container">
    <slideSearch :options="options">
      <div slot="btnBar">
        <el-button size="mini" type="primary" icon="el-icon-search" @click="getData(true)" @keydown.enter.native="getData(true)">查询</el-button>
        <el-button
          v-has-permi="['confinement:healthcareManagement:jianyanexport']"
          size="mini"
          type="success"
...
export default {
  ...
  methods: {
    ...
    customerclick(id, val) {
      this.$router.push({
        path: '/customerInfo',
        query: { id, status: val.customerStatus, noDeal: val.receptionStatus }
      })
    },
    // 获取数据
