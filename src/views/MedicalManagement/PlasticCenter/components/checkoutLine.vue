            ...
            <el-table border size="mini" :data="tableDatalist" :header-cell-style="{ 'text-align': 'center' }"
              ...
              <el-table-column label="单价" prop="priceOne">
                <template slot-scope="scope">
                  ...
                  <el-input-number v-model="scope.row.priceOne" :min="0.00" :max="10000000.01" size="mini" style="width: 100%"
              ...
              <el-table-column label="每次用量" prop="eachTime">
                <template slot-scope="scope">
                  <!-- <el-input-number
                    ...
                    @input="() => $forceUpdate()" />
                  <!-- <el-input type="text" @input="newchangepriceTwo($event, scope.row)" class="changePrice"
                    style=""></el-input> -->
                </template>
              </el-table-column>
              <el-table-column label="用药单位" prop="pharmacy">
                <template slot-scope="scope">
                  <el-select v-model="scope.row.unit"  @change="handleSelect(scope.row, $event)" filterable>
  ...
  methods: {
    ...
    addAdvice() {
      ...
      if (this.form.time == '' || this.StartTime == '') {
        this.$message.warning('请选择检验长嘱时间');
        return
      }
      let str = '';
      for (let i = 0; i < 8; i++) {
