            ...
            <el-table border size="mini" :data="tableDatalist" :header-cell-style="{ 'text-align': 'center' }"
              ...
              <el-table-column label="单价" prop="priceOne">
                <template slot-scope="scope">
                  ...
                  <el-input-number v-model="scope.row.priceOne" :min="0.00" :max="100000000.01" size="mini"
  ...
  methods: {
    ...
    orderNumber(val) {
      ...
      arr.forEach(item => {
        if (item.itemId === val.itemId) {
          ++item.priceNum
          item.totalPrices = item.priceNum * item.priceOne
          flat = true
        }
      })
