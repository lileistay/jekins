          ...
          <div class="formBody formBody_bot">
            <el-form ref="form" :model="form" size="mini" label-width="120px">
              ...
              <el-form-item label="给药途径">
                <el-select v-model="live" placeholder="请选择" size="mini" filterable>
              ...
              <el-form-item label="天数">
                <el-input-number v-model="form.days" style="width: 50px;" :min="1" />
              </el-form-item>
        ...
        <div class="middle">
          <div class="formBody formBody_middle">
            ...
            <sg-table ref="table" size="mini" border :pagination="pagination" highlight-current-row
              :table-data="tableData" :columns="columns" max-height="222"
            ...
            <el-table border size="mini" :data="tableDatalist" :header-cell-style="{ 'text-align': 'center' }"
              ...
              <el-table-column label="数量" prop="priceNum">
                <template slot-scope="scope">
                  <el-input-number  v-model="scope.row.priceNum"  size="mini" style="width: 100%"
                    @input="changepriceTwos($event, scope.row)" :min="1"  />
              ...
              <el-table-column label="单价" prop="priceOne">
                <template slot-scope="scope">
                  ...
                  <el-input-number v-model="scope.row.priceOne" :min="0.00" :max="100000.01" size="mini" style="width: 100%"
                    @input="changepriceTwo($event, scope.row)" />
                </template>
              </el-table-column>
              <el-table-column label="总价" prop="totalPrices">
                <template slot-scope="scope">
                  <el-input-number v-model="scope.row.totalPrices" :min="0.00" :max="100000.01" size="mini" disabled style="width: 100%"
                    @input="() => $forceUpdate()" />
                </template>
              </el-table-column>
              ...
              <el-table-column label="每次用量" prop="eachTime">
                <template slot-scope="scope">
                  <el-input-number v-model="scope.row.dosage" :min="0.1" :max="100000.01"  size="mini" style="width: 100%"
                    @input="() => $forceUpdate()" />
              ...
              <el-table-column label="用药单位" prop="dose_unit">
                <template slot-scope="scope">
                  <el-select filterable v-model="scope.row.unit"  @change="handleSelect(scope.row, $event)">
  ...
  methods: {
    ...
    addAdvice() {
      ...
      this.tableDatalist.forEach(value => {
        ...
        value.productItemType = value.projectTypeId
      ...
      ats.forEach(value => {
        ...
        //  value.itemId=value.categoryId;

      });
      // return
      // return
      //  whouseName// priceWhouse;
      //  expenseTypeName //expenseType
      //  executingState:'1',
      //  expense//总价
      //  doctorId
      //  doctorName
      // medicalAdviceType;categoryId
      // doseUnit//priceUnit

      // return
      // this.$store.getters.isReload='1231231';
