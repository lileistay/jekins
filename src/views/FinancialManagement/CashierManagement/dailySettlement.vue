<template>
    ...
    <div class="container">
        <slideSearch :options="options">
            <div slot="btnBar">
                ...
                <el-button v-has-permi="['payment:toBePaidList:export']" type="success" size="mini" icon="el-icon-upload2"
                    plain :loading="submitLoading" @click.stop="exportExcel">导出
                </el-button>
            </div>
            <slot>
                <sg-table id="out-table" ref="table" :row-key="handle" :sg-ref="'multipleTable'" size="mini" border
                    selection index="序号" :expand-row-keys="expands" highlight-current-row :table-data="tableData"
                    :columns="columns" :pagination="pagination"  @size-change="sizeChange"
                    @current-change="currentChange"  @selection-change="handleSelectionChange" :cell-class-name="cellClassName">
    ...
    placeorder < 0 ? -placeorder : 0 }}元</span> ]</span>&nbsp;
                    ...
                    <div slot="header" style="width: 100%;">
                        <span style="font-size: 14px;cursor:pointer"><i class="el-icon-s-fold" @click="getDialog" />
                            列表清单</span>
                        <div style="float: right">
                            <el-button type="primary" size="mini" plain @click="clickback" v-has-permi="['FinancialManagement:edit']">修改</el-button>
                        </div>
                    </div>
                    <!--字典调用-->
                    <template #custom="data">
                        <div v-if="data.props === 'billType'">
                            <show-dict-data :options="dict.type.billing_type"
                                :class="data.custom.billType == 11 ? 'redClass' : ''" :value="data.custom.billType" />
                        </div>
                        <div v-if="data.props == 'printtax'">
                            <el-button @click="editdaily(data.custom)" size="mini" type="text">修改</el-button>
                        </div>
                        <div v-if="data.props === 'paidAmount'">
                            <span :class="data.custom.billType == 11 ? 'redClass' : 'redClass1'"> {{ data.custom.paidAmount
                            }}</span>
                        </div>
                    ...
                    <template #expands="data">
                        ...
                        <span style="font-size: 14px;padding: 0 10px" v-has-permi="['payment:toBePaidList:paidList:listqd']"
                            v-if="data.expands.billType == 17 || data.expands.billType == 10">
                            <i class="el-icon-printer" style="color: #5f94de" />
                            <a style="color: #5f94de;cursor: pointer"
                                @click="printSetMealDetailsfy(data.expands)">费用结算清单</a>
                        </span>
                        <span style="font-size: 14px;padding: 0 10px" v-has-permi="['payment:toBePaidList:paidList:list']"
                            v-if="data.expands.billType == 17 || data.expands.billType == 10">
                            <i class="el-icon-printer" style="color: #5f94de" />
                            <a style="color: #5f94de;cursor: pointer"
                                @click="printSetMealDetailsfys(data.expands)">费用结算单</a>
                        </span>
                    </template>
                </sg-table>
            </slot>
        ...
        <vue-easy-print ref="orderDetailsPrint">
            <order-details-print :print="print" :message="newmessage" :details="details" :total="total"
        ...
        <vue-easy-print ref="setMealDetailsPrint">
            <set-meal-details-print :package-order-detail="packageDetailPrintData.packageOrderDetail"
                :package-items="packageDetailPrintData.packageItems"
                :payment-methods="packageDetailPrintData.paymentMethods" />
        </vue-easy-print>
            ...
            <div style="position: relative">
                ...
                <div class="order_detail_basic" style="justify-content: normal">
                    ...
                    <div style="width: 50%" v-if="vals.waitSource == '1'">
                        结束时间:{{ hosiptalData.leaveTime == undefined ? timeFmt('YYYY-mm-dd HH: MM: SS', new Date()) :
                            hosiptalData.leaveTime }}
                    </div>
                </div>

                <div>
                    <div style="width:700px" class="tablist">
                        <el-table :data="tableDataaccount" border :height="tableDataaccount.length == 0 ? 200 : ''">
                            <el-table-column prop="itemName" label="项目名称" align="center" width="100">
                            </el-table-column>
                            <el-table-column prop="amount" label="数量" align="center" width="50">
                            </el-table-column>
                            <el-table-column prop="materialSpec" label="规格" align="center"></el-table-column>
                            <el-table-column prop="doseUnit" label="单位" align="center"></el-table-column>
                            <el-table-column prop="unitPrice" label="单价" align="center"></el-table-column>
                            <el-table-column prop="discountAmount" label="优惠" align="center" />
                            <el-table-column prop="pricingTime" label="审核时间" align="center" width="145" />
                            <el-table-column prop="amountReceived" label="金额" align="center" width="80">
                            </el-table-column>
                        </el-table>
                    </div>
                ...
                <div class="order_detail_basic" style="display: flex;justify-content: space-between">
                    <div style="position: relative;width: 100%;display: flex;justify-content: space-between">
                        ...
                        <div>
                            <span>时间: <span style="margin-left:20px;"> {{ timeFmt('YYYY-mm-dd HH:MM:SS', new Date())
                            }}</span></span>
                        </div>
                    </div>
            ...
            <div style="position: relative">
                ...
                <div class="order_detail_basic">
                    <div style="width: 100%;min-height: 50px;display: flex;flex-wrap: wrap">
                        <div style="margin-top: 20px;margin-right: 20px;width: 45%;" v-for="item in listone"
                            v-if="item.payAmount > 0">{{ item.typeName }} : <span style="margin-left: 10px">{{
                                item.payAmount
                            }}</span> </div>
                    </div>
                </div>
                <div class="order_detail_basic" style="display: flex;justify-content: space-evenly">
                    <div style="position: relative">
                        <div style="">
                            <span v-if="this.vals.billType == '10' || this.vals.billType == '17'">预收:<span
                                    style="margin-left:20px;"> {{ this.ysmoney }}</span></span>
                        </div>
                    </div>
                    <div style="position: relative">
                        <div style="">
                            <span v-if="this.vals.billType == '10' || this.vals.billType == '17'">结算:<span
                                    style="margin-left:20px;"> {{ this.jsmoney }}</span></span>
                        </div>
                    </div>
                ...
                <div style="width: 100%;display: flex;justify-content: space-between;align-items: center;margin-top: 20px">
                    ...
                    <div style="font-size: 24px">
                        <span style="font-size: 16px">时间: <span style="margin-left:20px;"> {{ timeFmt('YYYY-mm-dd HH:MM:SS',
                            new Date()) }}</span></span>
                    </div>
        ...
        <sgDialog title="返日结" size="360px" :modal="false" :dialog="dailySettlement" @handleClose="dailySettlement = false">
            <div slot="out">
                <el-form :model="ruleForm" :rules="rulesdaily" ref="ruleForm" label-width="100px" class="demo-ruleForm">
                    <el-form-item label="返日结日期" prop="daily">
                        <!-- <el-date-picker  type="datetime" placeholder="选择日期时间" size="mini" clearable>
                        </el-date-picker> -->
                        <el-date-picker v-model="ruleForm.daily" type="date" placeholder="选择返日结日期" size="mini">
                        </el-date-picker>
                    </el-form-item>
                </el-form>
                <div class=""
                    ...
                    <div style="width: 160px;height: 40px;">
                        ...
                        <el-button @click="dailySettlement = false" size="mini">取消</el-button>
                    </div>
                </div>
            </div>
        </sgDialog>
        <sgDialog title="反日结修改" size="360px" :modal="false" :dialog="dailySettlementedit"
            @handleClose="dailySettlementedit = false">
            <div slot="out">
                <el-form :model="ruleFormedit" :rules="rulesdailyedit" ref="ruleFormedit" label-width="100px"
                    class="demo-ruleForm">
                    <el-form-item label="返日结日期" prop="daily">
                        <el-date-picker v-model="ruleFormedit.daily" type="date" placeholder="选择返日结日期" size="mini">
                        </el-date-picker>
                    </el-form-item>
                </el-form>
                <div class=""
                    style="position: absolute;width: 100%;height: 60px;bottom: 0;display: flex;align-items: center;justify-content: center;">
                    <div style="width: 160px;height: 40px;">
                        <el-button type="primary" size="mini" @click="okdailySettlementedit">确定</el-button>
                        <el-button @click="dailySettlementedit = false" size="mini">取消</el-button>
                    </div>
                </div>
            </div>
        </sgDialog>
...
export default {
    ...
    data() {
        return {
            listone: [],
            dailySettlementedit: false,
            rulesdaily: {
                daily: [
                    { required: true, message: '请选择返日结日期', trigger: 'blur' },
                ]
            },
            ruleForm: {
                daily: "",
    ...
    methods: {
      cellClassName({row,rowIndex}){
        ...
        okdailySettlementedit() {
            let daily = '';
            if(this.selectionData.length==1){
                daily=this.selectionData[0].orderNumber
            }else{
                this.selectionData.forEach((value,index) => {
                if(index==0){
                    daily+=value.orderNumber+','
                }else{
                    if(index==this.selectionData.length-1){
                        daily+=value.orderNumber
                    }else{
                        daily+=value.orderNumber+','
                    }
            ...
            //     fromdata.append('file', daily);
            this.$refs.ruleFormedit.validate((valid) => {
                if (valid) {
                    this.$confirm('确认返日结，是否继续？', '提示', {
                        distinguishCancelAndClose: true,
                        confirmButtonText: '同意',
                        cancelButtonText: '拒绝',
                        type: 'warning'
                    }).then((res) => {
                        this.$api.confinementRoom.backDayJunction({
                            backDayTime: this.newDate(this.ruleFormedit.daily).split(' ')[0],
                            orderNumber: daily
                        }).then(res => {
                            if (res.code == 200) {
                                this.$message({
                                ...
                                this.dailySettlementedit=false;
                                this.getData()
                            }
                    ...
                    // this.getData(true);
                    // this.options.checkoutTime.values=[this.ruleForm.daily,this.ruleForm.daily]
                } else {
        ...
        async getData(isSearch) {
            ...
            this.$set(data, 'deptId', this.deptId)
            this.$set(data, 'billingStatus', '3')
            let Time = this.newDate(this.ruleForm.daily);
            let newTime = Time.split(' ');
            data.checkoutTime = this.newDate(this.ruleForm.daily)
            data.endcheckoutTime = newTime[0] + ' ' + '23:59:59'
        ...
        clickback() {
            if (this.selectionData.length < 1) {
                this.$message.warning("请至少选择一条数据进行返日结修改");
                return
            }
            this.dailySettlementedit = true;
            this.ruleFormedit.daily='';
        },
    }
}
</script>
