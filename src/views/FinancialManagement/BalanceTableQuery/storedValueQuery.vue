...
<!--            />-->
      ...
      <div>
        <el-button size="mini" type="primary" @click="getData(true)" @keydown.enter.native="getData(true)">查询
        </el-button>
      </div>
    </div>
    <!--    <div style="background-color: #1abc9c; height: 35px;line-height: 35px">-->
    <!--      <p>卡储值查询列表</p>-->
    <!--    </div>-->
      ...
      <div class="left">
        ...
        >
          <template #custom="data">
            ...
            <div v-if="data.props === 'refundOut'">
              {{ Math.abs(data.custom.refundOut) }}
            </div>
            <div v-if="data.props === 'returningStored'">
              {{ Math.abs(data.custom.returningStored) }}
            </div>
            <div v-if="data.props === 'consumptionOut'">
              {{ Math.abs(data.custom.consumptionOut) }}
            </div>
          </template>
          <div slot="searchBar" class="searchBar" style="background-color: #1abc9c">
            <div class="rights">
              <p class="itemfont">储值卡查询列表</p>
            </div>
            <div class="rights">
              <!--                    <el-button size="mini">导出汇总</el-button>-->
              <el-button size="mini" style="margin-right: 10px" @click="exportExcel">导出</el-button>
            </div>
          </div>
        </sg-table>
      </div>
        ...
        <div v-if="rights">
          <div style="margin-bottom: 20px">
            ...
            >
              ...
              <el-table-column prop="orderNumber" width="160" label="收费单编号">
              ...
              <el-table-column prop="customPhone" width="110" label="电话"/>
              <el-table-column prop="billType" width="160" label="收费单类型">
                <template slot-scope="scope">
                  <dict-tag :options="dict.type.billing_type" :value="scope.row.billType"/>
                </template>
              </el-table-column>
              <el-table-column prop="amountPayable" label="结账金额">
                <template slot-scope="scope">
                  <p> {{ Math.abs(scope.row.amountPayable + scope.row.giftAmount )}} </p>
                </template>
              </el-table-column>
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          lab: '退返储值',
          val: 'returningStored',
          custom: 'returningStored'
        },
        {
          lab: '消费出',
          val: 'consumptionOut',
          custom: 'consumptionOut'
        },
        {
          lab: '退款出',
          val
  ...
  methods: {
    ...
    exportExcel() {
      ...
      this.$api.finance.rechargeConsumptionInfo(data).then(res => {
        if (res) {
          this.tableData = res.rows
        }
      }).finally(() => {
        export_table_to_excel('#out-table', '储值卡查询列表')
      })
    },
    aesFilter(val) {
      if (val) {
    ...
    handleCurrentChange(val) {
      ...
      this.$api.billingList.storedRecordList(query).then(res => {
        if (res) {
          this.tableList = []
          this.ListData = []
