      ...
      <div class="left">
        ...
        >
          <div slot="searchBar" class="searchBar" style="background-color: #1abc9c">
            ...
            <div class="rights">
              <el-button style="margin-right: 10px" size="mini">导出</el-button>
            </div>
          </div>
        </sg-table>
      </div>
...
<!--            <el-table-column prop="date" label="原收费单编号" />-->
            <el-table-column prop="customerName" label="姓名" />
            <el-table-column prop="customPhone" label="电话" />
            <el-table-column prop="billType" label="收费单类型">
              <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    getData: debounce(function(isSearch) {
      ...
      this.$api.finance.advanceConsumptionRecord(data).then(res => {
        if (res) {
    ...
    async handleCurrentChange(val) {
     ...
     var id = val[1].id
      await this.$api.billingList.appentRecord(data).then(res => {
        if (res) {
          res.data.forEach(res => {
            res.customerName = val[1].customerName
            res.customPhone = val[1].customPhone
            if (res.billType === '2' || res.billType === '4') {
