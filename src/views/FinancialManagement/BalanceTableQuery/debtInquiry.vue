      ...
      <div class="right">
        <div v-if="rights">
          <div style="margin-bottom: 20px">
            <div class="rights">
              <p class="itemfont">欠款明细</p>
            </div>
            <el-table :data="tableList" max-height="300px" size="mini" style="width: 100%" :cell-class-name="cellClassName">
              ...
              <el-table-column prop="deptPrice" label="结账金额" />
              <el-table-column prop="dsingleUserName" label="下单人" />
            </el-table>
          </div>
          <div class="rights">
            <p class="itemfont">还款明细</p>
          </div>
          <el-table :data="ListData" max-height="300px" size="mini" style="width: 100%" :cell-class-name="newcellClassNames">
            <el-table-column label="序号" type="index" width="50" />
            <el-table-column prop="orderNumber" width="165" label="收费单编号">
              <template slot-scope="scope">
                <a style="color: #5f94de;cursor: pointer;" @click="orderNumber(scope.row)">
...
export default {
  ...
  methods: {
    ...
    exportExcel(){
        ...
        this.$api.finance.selectSumStatisticsList(data).then(res => {
            if (res) {
                this.tableData = res.rows
            }
        }).finally(() => {
            export_table_to_excel('#out-table', '欠款查询列表')
        })
    },
    aesFilter(val) {
      if (val) {
    ...
    handleCurrentChange(val) {
      ...
      // 欠款明细
        this.$api.billingList.debtDetailsList(data).then(res => {
        if (res) {
          // res.rows.forEach(res => {
          //   res.customerName = val[1].customerName
          //   res.customPhone = val[1].customPhone
          // })
            this.tableList = res.data
        }
      })
      // 还款明细
        this.$api.billingList.repaymentOrderDetails(data).then(res => {
        if (res) {
          // res.rows.forEach(res => {
          //   res.customerName = val[1].customerName
          //   res.customPhone = val[1].customPhone
