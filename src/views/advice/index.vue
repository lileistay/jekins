<template>
  <div>
    <!-- <div class="nurse_top">
      <span @click="proofread">医嘱停止</span>
    </div> -->
    <div class="nurse_top" style="position: relative;">
     <div style="position: absolute;right:50px;top:3px">
        <el-button type="primary" plain size="mini" @click="proofread" style="height: 25px;line-height: 10px;">医嘱停止</el-button>
    </div>
    </div>
    <div class="nurse_contain" style="margin-top: 4px;">
      <div class="nurse_left">
        <div class="nurse_search">
          ...
          <el-input placeholder="请输入姓名" size="mini" style="width: 160px" v-model="Name" autocomplete='off'></el-input>
        ...
        <div class="patient_list">
          <div style="width: 300px;">
            <el-table :data="Nurse_left" stripe :row-style="{ height: '0' }" :cell-style="{ padding: '3px' }"
              :header-cell-style="{ background: '#eef1f6', color: '#606266' }" @row-click='selechColum' :height="heightstop" highlight-current-row ref="tablelist">
      ...
      <div class="nurse_right">
        <div class="nurse_right_top" style="background-color: antiquewhite;">
          ...
          <div style="display: flex;" >
                            ...
                            <div class="topContain"> 病区床位：<span>{{ customerNameList.wardNo }}</span></div>
    ...
    customerNameList.remark }}</span>&emsp;&emsp;押金警示：<span>暂无</span> -->
        ...
        <div>
          ...
          <span style="margin-left:10px">停止人：</span>
          <el-select filterable v-model="Names" placeholder="请选择" size="mini" @change="changeStop">
            <el-option v-for=" (item, index) in newoptions" :key="index" :label="item.nickName" :value="item.userId">
            </el-option>
          </el-select>
          <span>停止时间：</span>
          <el-date-picker v-model="value1" type="date" placeholder="选择日期" size="mini"  :editable="false">
          </el-date-picker>
        </div>
        <div class="patient_list_right" style="margin-top: 12px;">
          <div style="width:100%;">
            <el-table :data="tableData" stripe ref="table" :row-style="{ height: '0' }" :cell-style="{ padding: '3px' }"
              ...
              @selection-change="handleSelectionChange" @row-click='selechColums' :height="newheight">
...
export default {
  ...
  mounted() {
    ...
    this.value1=this.timeFmt('YYYY-mm-dd', new Date())
    let data = {
      status: 0,
      deptId:this.deptId
    }
  ...
  if (!time) {
    ...
    selechColum(row, column, event) {
            ...
            this.$api.user.getUserList(datas).then(res => {
                ...
                let data = { inHospitalId: this.inHospitalId, executingState: '6' ,deptId:this.deptId}
    ...
    changeStop(val) {
      ...
      // let data = { inHospitalId: this.inHospitalId, executingState: '6' ,deptId:this.deptId,applyStopName:val,applyStopTime:this.value1==''?'': this.newDate(this.value1)+' '+'00:00:00'}
      //   this.$api.confinementRoom.listGroup(data).then(res => {
      //     let Data = [];
      //     res.rows.forEach(value => {
      //       value.forEach(values => {
    ...
    proofread() {
      if (this.Rows.length == 0) {
        this.$message.warning('请选择一条停止数据');
        return
      }
      let data = this.Rows;
      let stopName;
    ...
    this.Names=this.NameId
      ...
      this.newoptions.forEach(value=>{
        if(value.userId==this.Names){
          stopName=value.nickName
        }
      })
      if(stopName==undefined){
         this.$message.warning('请选择停止人');
         return
      }
      if(this.value1==''||this.value1==null){
        this.$message.warning('请选择停止时间');
        return
      }

      data.forEach(value => {
        value.executingState = '7';
        value.stopId = this.Names;
        value.stopName=stopName;
        value.stopTime=this.newDate(this.value1)+' '+"00:00:00" ;
      })
