    ...
    <div>
        <div class="order">
            <div class="ORDER_CHILDREN">
              ...
              <div class="order-children" style="width: 300px;" v-show="value1">
                <span>订单类型:</span>
                <el-select v-model="valuetype" placeholder="请选择" size="mini" class="Left" filterable clearable
                ...
                <div class="order-children" style="width: 650px;">
                    <span>时间类型:</span>
                    <el-select v-model="value" placeholder="请选择" size="mini" class="Left" filterable clearable @change="changeselect">
...
<!--                    </el-date-picker>-->
                  <el-date-picker
                    v-model="value3" class="inputWidth"
                    size="mini"
                    ...
                    :picker-options="pickerOptions" :default-time="['00:00:00', '23:59:59']" style="width: 290px;margin-left: 50px" />
                </div>

                <div class="order-children" style="width: 545px;">
                    <span>查询类型:</span>
                    <el-select v-model="value2" placeholder="请选择" size="mini" class="Left" filterable clearable>
        ...
        <div style="display: flex;margin-left: 20px;margin: 10px;justify-content: end;">

            <h3>订单GWV：</h3><span style="margin-top: 3px;" v-if="totalMap">{{ totalMap.tradedPayment }}</span>&nbsp;&nbsp;&nbsp;&nbsp;
            <h3>未成交金额：</h3><span style="margin-top: 3px;" v-if="totalMap">{{ totalMap.unsettledPayment }}</span>&nbsp;&nbsp;&nbsp;&nbsp;
            <h3>已退款金额：</h3><span style="margin-top: 3px;"  v-if="totalMap">{{ totalMap.refundPayment }}</span>
        </div>
        <div class="order_contains">
            <div class="text">
                订单信息
            </div>
            <div class="text">
        ...
        <div style="height: 670px;overflow-y: scroll;">
            <div v-for="(item, index) in listorder">
                <div class="order_contain"
                    style="background-color: #f4f8ff;height: 30px;padding-left: 12px;line-height: 30px;position: relative;">
                    <span>{{ item.productType==1?'实物':'服务' }}</span>
                    <span class="Left">下单时间:{{ item.createTime }} </span>
                    <span class="Left">订单号:{{ item.orderNumber }}</span>
                    <span class="Left">付款方式:{{ item.payType == undefined ? '' : item.payType == 0 ? '全款' : '预约金' }} </span>
                    <span class="Left">订单来源门店:{{ item.deptName }} </span>
                    ...
                    <div style="width:380px;right:0%;position: absolute;">
                ...
                <div class="order_contain" style="background-color: #fff;height: 120px">
                    <div class="order_detail Top"
                        ...
                        <img :src="item.imgurl" alt="" style="width: 87px;height: 87px;">
                        ...
                        <span class="Left" v-if="item.productType=='1'&&item.orderState=='2'">发货单号：<span style="color:red">{{ item.expressNumber }}</span></span>
                        <span class="Left" v-if="item.productType=='1'&&item.orderState=='2'">配送方式：<span style="color:red">{{ item.deliveryType==0?"自提":'配送' }}</span></span>
                    </div>
                    <div class="order_detail Top"
                        ...
                        <div style="position: absolute;top:0px;">
                            ...
                            <p style="height: 25px;">
                                <span class="Height" style="height:10px;">买家备注：{{ item.buyerRemarks }}</span>
                            </p>
                    ...
                    <div class="neworder_detail Top"
                        ...
                        <el-button type="text" size="mini" @click="backorder(item, index)"
                            v-if="item.orderState == '4'">同意/拒绝退款申请</el-button>
                        <el-button type="text" size="mini" @click="writesay(item, index)"
                            v-else-if="item.orderState == '1'&&item.deliveryType!=='0'">填写发货单号</el-button>
                        <el-button type="text" size="mini" @click="say(item, index)" v-else>订单详情</el-button>
                    </div>
                </div>
            </div>
        ...
        <sgDialog title="订单详情" size="1050px" out-top="2%" :dialog="showDialogtooth" @handleClose="showDialogtooth=false">
            <div slot="out" style="width:100%;height:800px;background:#f4f8ff;overflow:hidden">
                ...
                <div class="order_top" :style="{ height: Index == 3 ? '200px' : '' }">
                    ...
                    <div style="display:flex;margin-left:20px;margin-top:20px" v-show="Index == 5">
                        ...
                        <div style="position:absolute;margin-top:50px" v-if="backmonry">
                            <span>退款金额：<span style="color:red">￥{{ backmonry.refundAmount }}</span></span> <span>操作人：{{ backmonry.approverName}}</span>
                            <p>退款时间：{{ backmonry.approverTime }}</p>
                            <p>申请人：{{ backmonry.customerName }}</p>
                            <p>申请时间：{{ backmonry.refundTime }}</p>
                ...
                <div class="order_top" style="height:300px;background:#f4f8ff">
                    <div class="order_left" style="background:#fff;height:300px;">
                        ...
                        <div style="margin-top:20px;margin-left:30px">
                            <div style="display: flex;align-items: center;">
                                <span class="Tops ">订单编号:<input id="copyOrdernumbers" :value="Items.orderNumber"
                                        style="border: none;outline: none;width: 135px;" /></span> <img
                                    src="@/assets/images/copy.png" class="IMG" @click="copyOrdernumber(Items.orderNumber)">
                            </div>

                            <p class="Tops">下单时间: {{ Items.createTime }}</p>
                            <p class="Tops">商品类型:<span>{{ Items.payType }}</span>  商品</p>
                            <!-- <p class="Tops">下单门店: 兰州时光整形美容医院</p>
                            <p class="Tops">适用门店: 全部门店</p> -->
                            <p class="Tops">付款方式: {{ Items.payType == undefined ? '' : Items.payType == '0' ? '全款' : '预约金'
                            }}</p>
                    ...
                    <div class="order_left" style="background:#fff;height:300px;margin-left:1%">
                        ...
                        <div style="margin-top:20px;margin-left:30px">
                            <div style="display: flex;align-items: center;">
                                <span class="Tops">客户姓名:{{ Items.customerName }}</span> <img
                                    src="@/assets/images/person.png" class="IMG" @click="personal">
                            </div>
                ...
                <div class="order_tops" style="height:200px">
                    ...
                    <div style="display:flex">
                        <img :src="ImgUrl" style="width:104px;height:104px;margin-left:20px;margin-top:20px;text-align:top">
        ...
        <sgDialog title="发货单号" size="18%" out-top="2%" :dialog="witreshowDialogtooths"
            @handleClose="writecloseRoomDialogtooths">
            <div slot="out" style="width:100%;height:100px;overflow:hidden">
                <span style="color:red">*</span>
                <span>发货单号</span>
                <el-input size="mini" style="width:200px;height:30px;margin-left:20px" v-model="fhhxm"></el-input>

                <div class=""
                    style="position: absolute;width: 100%;height: 60px;bottom: 0;display: flex;align-items: center;justify-content: center;">
                    <div style="width: 160px;height: 40px;">
                        <el-button type="primary" size="mini" @click="writeok">确定</el-button>
                        <el-button @click="witreshowDialogtooths = false" size="mini">取消</el-button>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        writeok() {
            ...
            this.$api.confinementRoom.onlineOrderInfowrite({
                orderInfoId: this.writeorder.orderInfoId,
                expressNumber: this.fhhxm,
                orderState: "2",
            }).then(res => {
                if (res.code == 200) {
                    this.$message.success("发货单号填写成功");
                    this.search();
                    this.witreshowDialogtooths = false;
                }
            })
        },
        writecloseRoomDialogtooths() {
            this.witreshowDialogtooths = false;
        },
        writesay(item, index) {
            this.writeorder = item
            this.fhhxm = '';
            this.witreshowDialogtooths = true;
        },
        changetypevalue(val) {
            if (val == '2') {
                this.alllist[0].value = [{ name: '订单状态', border: false }, { name: '不限', border: true, back: "#1abc9c", color: "#fff", index: "" }, { name: '待支付', border: true, back: "", color: "", index: 0 },
                { name: '待核销', border: true, back: "", color: "",index:1 },
                { name: '待退款', border: true, back: "", color: "",index:4 },
                { name: '已退款', border: true, back: "", color: "",index:5 },
                { name: '已取消', border: true, back: "", color: "",index:6 }
            
            ]
            } else {
                this.alllist[0].value = [{ name: '订单状态', border: false }, { name: '不限', border: true, back: "#1abc9c", color: "#fff", index: "" }, { name: '待支付', border: true, back: "", color: "", index: 0 },
                { name: '待发货', border: true, back: "", color: "", index: 1 },
                { name: '已发货', border: true, back: "", color: "", index: 2 },
                { name: '已支付', border: true, back: "", color: "", index: 7, },
        ...
        search() {
            ...
            data.productType = this.valuetype;
        ...
        say(item, index) {
            ...
            this.$api.confinementRoom.getInfoByOrderInfoId(data).then(res => {
                this.backmonry=res.data
            })
            ...
            alllist: [
                {
                    ...
                    { name: '待发货', border: true, back: "", color: "", index: 1 },
                    { name: '已发货', border: true, back: "", color: "", index: 2 },
                    //   { name: '已支付', border: true, back: "", color: "",index:3},
                    //    { name: '待核销', border: true, back: "", color: "" },
                    { name: '待退款', border: true, back: "", color: "", index: 4 },
                    { name: '已退款', border: true, back: "", color: "", index: 5 },
                    { name: '交易已完成', border: true, back: "", color: "", index: 3 },
