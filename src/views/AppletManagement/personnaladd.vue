    ...
    <div>
        <div style="height:780px;width:100%;overflow-y:scroll">
            <div style="margin-left: 80px;margin-top: 50px;display: flex;flex-wrap: wrap;">
                <div style="position: relative;margin-top:15px" v-for="(items, indexs) in uploadlist.slice(0, 11)"
                    :key="indexs">
                    ...
                    <div v-if="items.name !== 'logo'">
                        ...
                        <div v-show="items.name == 'jy'">
                            ...
                            <el-select filterable v-model="items.row" placeholder="请选择" size="mini" clearable
                        ...
                        <div v-show="items.name == 'qz'" class="max">
                            ...
                            <span
                                :style="{ fontSize: '14px', marginLeft: items.Name == '微信' || items.Name == '开启预约' || items.Name == '是否默认' ? '10px' : '' }">{{
                                    items.Name }}</span>
                            <el-input v-model="items.row" :style="{ width: '250px', marginLeft: '10px' }"
                                :maxlength="items.num" show-word-limit size="mini"
                                v-if="items.Name == '电话' || items.Name == '微信'"></el-input>
                            <el-switch v-model="valueselect" active-color="#13ce66" inactive-color="#ff4949"
                                v-if="items.Name == '开启预约'" style="margin-left: 20px;">
                            </el-switch>
                            <el-switch v-model="defaultFlag" active-color="#13ce66" inactive-color="#ff4949"
                                v-if="items.Name == '是否默认'" style="margin-left: 20px;">
                            </el-switch>
            ...
            <div v-for="(items, indexs) in uploadlist.slice(11, 12)" :key="indexs"
                style="margin-left: 80px;display: flex;margin-top: 40px;">
                <div style="color: red;font-size: 14px;">*</div>
                <div style="font-size: 14px;">形象照</div>
                ...
                <div class="busic_img">
                    <div v-for="(item, index) in items.value" :key="index" class="busic_img_children"
                        @mouseover="newdelete(index, items.name, indexs + 11)"
                        @mouseleave="newleave(index, items.name, indexs + 11)"
                        :style="{ marginLeft: index % 2 == 0 ? '0px' : '10px', marginTop: '10px', border: item.border, width: items.width, height: items.height }"
                        @click="addlist(index, 'title_img' + items.name, indexs + 11)">
                        ...
                        <div class="title_del" v-show="item.showflag"
                            @click="deleter($event, index, items.name, indexs + 11)">
                        ...
                        <div class="title_success" v-show="item.flags" style="z-index: 100000;">
                            <img src="@/assets/images/ok.png" alt="" style="width: 100%;height: 100%;margin-top: 10px;">
                        </div>
                    </div>
            ...
            <div style="position: absolute;right: 20px;top:0px;background-color: #fff;height: 50px;">
                <el-button type="primary" size="mini" style="position: absolute;right:120px;top:145px;z-index:1000"
                    @click="submit" :loading="submitcompant" v-has-permi="['client:companyEmployeeInfo:add']"
                    v-if="showperson == 'false'">
                    提交
                </el-button>
                <el-button type="primary" size="mini" style="position: absolute;right:120px;top:145px;z-index:1000"
                    @click="submit" :loading="submitcompant" v-has-permi="['client:companyEmployeeInfo:edit']"
                    v-if="showperson == 'true'">
                    提交
                </el-button>
...
export default {
    ...
    created() {
        ...
        let flag = localStorage.getItem('personnaladd');
        this.showperson = localStorage.getItem('personnaladd');
      

        let id = localStorage.getItem('personnaladdId')
        if (flag == 'true') {
            ...
            ids.employeePhoto.forEach(value => {
            ...
            if (ids.employeePhoto == null) {

            } else {

                this.uploadlist[11].value[0].imgUrl = serverPath + object.logo[0].photoAddr;
                this.fileList['logo'] = object.logo;
                this.uploadlist[11].value[0].flags = true;
            }
            ...
            this.uploadlist[0].row = ids.employeeName;
            this.uploadlist[2].row = ids.professionGrade;
            this.valueselect = ids.reservationFlag == 1 ? true : false
            this.defaultFlag = ids.defaultFlag == 1 ? true : false;
            this.uploadlist[3].row = ids.jobDuration;
            this.uploadlist[5].row = ids.certificationNum;
            this.weightSort = ids.weightSort;
            this.uploadlist[6].row = ids.employeePhone;
            this.uploadlist[7].row = ids.employeeWx;
            ...
            // console.log(this.uploadlist[0].value[0].imgUrl)
    ...
    methods: {
        ...
        submit(val) {
            ...
            // if (this.uploadlist[6].row == '') {
            //     this.$message.warning('请输入电话');
            //     return;
            // } else {
            //     if (!validPhone(this.uploadlist[6].row)) {
            ...
            if (this.uploadlist[11].value[0].imgUrl == '') {
                this.$message.warning("请上传形象照");
                return
            }
            ...
            let ids = JSON.parse(id);
            let date = {
                employeeName: this.uploadlist[0].row,
                reservationFlag: this.valueselect == true ? '1' : '0',
                professionGrade: this.uploadlist[2].row,
                jobDuration: this.uploadlist[3].row,
                adept: str,
                employeePhone: this.uploadlist[6].row,
                employeeWx: this.uploadlist[7].row,
                introduce: this.info_,
                deptId: this.deptId,
                employeeId: ids==null||ids==undefined?'':ids.employeeId,
                labelList: this.tags,
                photoInfo: arrstr,
                defaultFlag: this.defaultFlag == true ? '1' : 0,
            }
            if (flag == 'true') {
                this.submitcompant = true;
                this.$api.confinementRoom.companyEmployeeInfoedit(date).then(res => {
                    if (res.code == 200) {
                        this.submitcompant = false;
                        this.$message.success('修改个人名片信息成功');
                        vm.$emit('closeTag', this.$route.path);

                    }
                });
            } else {
                this.$api.confinementRoom.companyEmployeeInfo(date).then(res => {
                    if (res.code == 200) {
                        this.$message.success('新增个人名片信息成功');
                        this.submitcompant = false;
                        vm.$emit('closeTag', this.$route.path);
    ...
    data() {
        return {
            ...
            newHonorable: [
                ...
                {
                    name: "operation", Name: "资质证明", desc: '建议图片不超过2MB，建议图片尺寸：248*164',
                    value: [{
                        imgUrl: "",
                        border: '1px dashed #d9d9d9',
                        showflag: false,
