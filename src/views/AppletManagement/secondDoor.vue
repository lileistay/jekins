    ...
    <div>
      ...
      >
        ...
        <template #custom="data">
          ...
          <div v-if="data.props==='roomTypeImage'" style="display:flex;align-items:center;justify-content:center">
      ...
      <sgDialog :title="dialogTitle" size="400px" :dialog="showDialog" @handleClose="handleClose">
        <div slot="out">
          <el-form ref="form" :rules="rules" :model="form" size="mini" label-width="120px">
            <el-form-item label="门店类型" prop="roomTypeName">
              <el-input v-model="form.roomTypeName" placeholder="请输入门店类型" />
            </el-form-item>
            <el-form-item label="门店类型图片" prop="roomTypeImage">
              <el-input v-model="form.roomTypeImage" placeholder="请上传门店类型图片" @click.native="seleImg" />
            </el-form-item>
            <el-form-item>
    ...
    data() {
      return {
        ...
        rules: {
          ...
          roomTypeImage:[
            { required: true, message: '请上传门店类型图片,建议375*300', trigger: 'blur' }
          ]
        },
        // 分页 搜索
        pagination: {
    ...
    methods: {
        ...
        changeImg(e) {
            ...
            this.$api.confinementRoom.unload(fromdata).then(res => {
                if (res.code == 200) {
                    this.form.roomTypeImage = res.data.url;
      ...
      async handleUpdate(e) {
        this.form={
            ...
            roomTypeImage:e.roomTypeImage,
            roomTypeId:e.roomTypeId
        }
        this.showDialog = true
        this.dialogTitle = '修改二级门店'
      ...
      handleDelete(e) {
        this.$confirm('是否确认删除数据？', '警告', {
          ...
          await this.$api.confinementRoom.roomTypeInforemove(e.roomTypeId).then(res => {
            if (res) {
              this.getData()
      ...
      getData: debounce(function(isSearch) {
        ...
        this.$api.confinementRoom.roomTypeInflist({
            storeId:this.$route.query.storeId,
        }).then(res => {
          if (res) {
            res.rows.forEach(value=>{
                value.imgurl=serverPath+value.roomTypeImage
            })
        ...
        await this.$refs.form.validate(async valid => {
          if (valid) {
            if(this.form.roomTypeId!==''){
                await this.$api.confinementRoom.roomTypeInfoedit(this.form).then(res => {
              if (res) {
                this.$message.success(res.msg)
                this.showDialog = false
                this.getData()
                ...
                await this.$api.confinementRoom.roomTypeInfo(this.form).then(res => {
              if (res) {
                this.$message.success(res.msg)
                this.showDialog = false
                this.getData()
