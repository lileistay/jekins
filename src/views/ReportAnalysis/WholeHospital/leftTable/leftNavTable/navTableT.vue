      ...
      <div>
        <el-form ref="formdept" size="mini" :inline="true" :model="form" label-width="80px">
          ...
          <el-form-item label="上级类型" prop="deptId">
            <el-select v-model="form.projectTypeId" filterable placeholder="请选择" @change="getConstructorName">
              <el-option
    ...
    >
      <template #custom="data">
        <div v-if="data.props === 'queryName'">
          <a style="color: #3aa1ff;cursor: pointer;" @click="clium(data.custom)">{{ data.custom.queryName }}</a>
        </div>
      </template>
    </cellTable>
  </div>
...
import navTable from '@/views/ReportAnalysis/WholeHospital/leftTable/leftNavTable/navTable'
...
export default {
  ...
  methods: {
    getConstructorName(val){
      this.itemName=this.depertmentList.filter(item=>item.projectTypeId==val)[0].projectTypeName
    },
    async getTreeselect() {
    ...
    getDepartment() { // 获取科室接口
      ...
      this.$api.modules.filing.getProjectTypeInfoList(data).then(res => {
        if (res) {
    ...
    clium(val) {
      ...
      this.$set(val, 'projectTypeId', this.form.projectTypeId)
      this.$set(val, 'categoryId', val.queryId)
      this.$set(val, 'timeData', this.form.timeData)
      this.$router.push({ path: 'threeheader', query: { item: val }})
    },
    getlist() {
      var data = {
        departmentId: this.form.departmentId,
