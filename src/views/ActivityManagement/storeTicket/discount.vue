      ...
      <slideSearch :options="options">
        <div slot="btnBar">
          <el-button size="mini" icon="el-icon-search" type="primary" plain @click.stop="getData(true)"
            @keyup.enter.native="getData(true)">查询
          </el-button>
          ...
          <sg-table id="out-table" v-if="true" ref="table" :sg-ref="'multipleTable'" :pagination="pagination" size="mini" selection border
            ...
            <template #custom="data">
              ...
              <div v-if="data.props === 'typeThreeName'">
                  <a href="#" @click="addrouter(data.custom)">发放详情</a>
              </div>
  ...
  export default {
    ...
    methods: {
      addrouter(val){
       ...
       localStorage.setItem('addrouterId',val.couponId)
      },
      editnews(val){
      ...
      */
      ...
      exportExcel() {
        ...
        this.$api.confinementRoom.listlong(data).then(res => {
          if (res) {
            this.$message({
              message: '稍等一下，马上就好',
              type: 'success'
          ...
          export_table_to_excel('#out-table', '优惠券')
          setInterval(() => {
