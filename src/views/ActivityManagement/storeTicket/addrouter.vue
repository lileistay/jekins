<template>
    ...
    <slideSearch :options="options">
      <div slot="btnBar">
        <el-button size="mini" icon="el-icon-search" type="primary" plain @click.stop="getData(true)"
                   @keyup.enter.native="getDatakeyup">查询
        </el-button>
  ...
  <el-button v-hasPermi="['confinement:customerCmt:edit']" size="mini" type="info" plain icon="el-icon-download"
    class="headerBarBtn" @click.stop.native="importCustomer">导入顾客信息
  </el-button>
  <el-button v-hasPermi="['confinement:customerCmt:edit']" size="mini" type="primary" plain icon="el-icon-plus"
    class="headerBarBtn" @click.stop.native="handlecompile">修改顾客信息
  </el-button>
  <el-button v-has-permi="['Appointment:modification']" size="mini" type="success" plain icon="el-icon-plus"
    class="headerBarBtn" @click.stop.native="appointmentAddition">添加预约
  </el-button> -->
            <!-- <el-button v-hasPermi="['sysManger:dept:Handmade']" size="mini" type="primary" plain icon="el-icon-plus"
    class="headerBarBtn" @click.stop.native="openManualDialog">添加优惠券
  </el-button> -->
            <!-- <el-button v-has-permi="['sysManger:dept:label']" size="mini" type="primary" plain icon="el-icon-edit-outline"
    class="headerBarBtn" @click.stop="customerLabelEdit">客户标签
  </el-button> -->
          </div>
          <template #custom="data">
            <div v-if="data.props === 'usageState'">
              {{
                data.custom.usageState == 0 ? '未使用' : data.custom.usageState == 1 ? '已使用' : data.custom.usageState == 2 ? '已冻结' : '已过期'
              }}
            </div>
            <div v-if="data.props === 'couponType'">
              {{ newtype(data.custom.couponType) }}
            </div>
            <div v-if="data.props === 'validityEndTime'">
              <div>{{ data.custom.validityStartTime }}</div><span>~</span>
              <div>{{ data.custom.validityEndTime }}</div>
            </div>

            <div v-if="data.props === 'useTimeType'">
              {{ newtypes(data.custom.useTimeType) }}
            </div>
            <div v-if="data.props === 'typeThreeName'">
              <a href="#" @click="addrouter">发放详情</a>
            </div>
            <div v-if="data.props === 'couponState'">
              {{ data.custom.couponState == 0 ? '上架' : '下架' }}
            </div>
            <div v-if="data.props === 'pregnantWeek'">
              <a href="#" @click="editnews(data.custom)">{{ data.custom.couponState == 0 ? '停用' : '启用' }}</a>
            </div>
            <!-- couponInfoedit -->
            <!-- 点击姓名跳转路由 -->
              ...
              <el-row>
                ...
                <el-col :span="12">
                  <el-form-item label="经济能力">
                    <el-select filterable v-model="manualForm.userInfo.economicCapacity" class="full-width"
                               placeholder="请选择">
              ...
              <el-row>
                <el-col :span="12">
                  <el-form-item label="标签类型">
                    <el-select filterable v-model="manualForm.form.customerLabelTypeId" class="full-width"
                               placeholder="请选择标签类型" @change="labelTypeSelection">
...
export default {
  name: 'BookFormQuery',
  dicts: ['marital_status', 'sys_user_sex', 'hospat_yn', 'economic_capacity', 'execution_type', 'return_stage', 'occupation', 'whether', 'discount_type', 'Issue_type', 'Enable'],
  components: {
    SendMessage,
    ManualSubmission,
    SgDialog,
  ...
  data() {
    return {
      ...
      columns: [
        ...
        //     lab: '发放详情',
        //     val: 'typeThreeName',
        //     custom: 'typeThreeName',
        //     width: 140
        //   },
        //   {
        //     lab: '优惠券开始时间',
        //     val: 'releaseStartTime',
        //     width: 200,
  ...
  created() {
    this.options.usageState.options = [
      {
        value: "0",
        label: "未使用"
      },
    ...
    this.options.couponType.options = this.dict.type.discount_type
    // this.options.createuserId.options=this.dict.type.Issue_type;
    this.options.couponState.options = this.dict.type.Enable
  ...
  methods: {
    newtype(id) {
      let str = ''
      this.dict.type.discount_type.forEach(value => {
        if (value.value == id) {
          str = value.label;
        }
      })
    ...
    addrouter() {
      this.$router.push('/addrouter')
    },
    editnews(val) {
      if (val.couponState == '1') {
        this.$confirm('启用该优惠券，是否继续？', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          val.couponState = '0';
          this.$api.confinementRoom.couponInfoedit(val).then(res => {
            this.$message({
              type: 'success',
              message: '启用成功!'
            });
    ...
    getData: debounce(function (isSearch) {
      ...
      this.showDataDialog = true;
      let addrouterId = localStorage.getItem('addrouterId');
      data.couponId = addrouterId
      this.$api.confinementRoom.couponReceive(data).then(res => {
        if (res) {
          this.tableData = res.rows
