        ...
        <div>
          <el-form size="mini" :model="form" label-width="120px"  :inline="true" :show-message="false">
            <el-form-item label="礼品类型：">
              <el-select filterable v-model="form.giftType" style="width: 140px;"  placeholder="请选择" clearable>
                <el-option
            ...
            <el-form-item label="执行人：">
              <el-select v-model="form.user" style="width: 140px;"  placeholder="请选择" filterable clearable :filter-method="may">
                <el-option
...
<!--              </el-date-picker>-->
      ...
      <template #custom="data">
        <div v-if="data.props === 'changeState'">
          <div v-if="data.custom.changeState==0">入库</div>
          <div v-if="data.custom.changeState==1">出库</div>
          <div v-if="data.custom.changeState==2">兑换</div>
        </div>
        <div v-if="data.props === 'edit'">
          <el-button type="text"  @click="editDialogs(data)">编辑</el-button>
        </div>
      </template>
    </sg-table>
  </div>
...
export default {
  ...
  methods:{
    ...
    getInOutInventoryList(){
      ...
      this.$api.modules.activityManagement.giftIncomingOutgoingInfoList(query).then(res => {
        if (res) {
