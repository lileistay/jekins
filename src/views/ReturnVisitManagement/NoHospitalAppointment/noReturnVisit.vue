  ...
  <div class="container">
    <slideSearch :options="options">
      <div slot="btnBar">
        ...
        >查询</el-button>
        <el-button
          v-has-permi="['confinement:returnVisitRecord:export']"
          type="success"
          size="mini"
      ...
      <slot>
        ...
        >
          ...
          <div slot="header" style="width: 100%;">
            ...
            <el-button
              v-has-permi="['noReturnVisit:remove']"
              size="mini"
              type="danger"
              plain
              icon="el-icon-delete"
            ...
            >删除</el-button>
            <el-button
              v-has-permi="['noReturnVisit:Reassign']"
              size="mini"
              type="primary"
              plain
            ...
            >未回访重新分配</el-button>
            <!--            <el-button-->
            <!--              v-hasPermi="['noReturnVisit:informations']"-->
            <!--              size="mini"-->
            <!--              type="warning"-->
            <!--              plain-->
            ...
            <!--            >批量发送短信</el-button>-->
            <el-button
              v-has-permi="['noReturnVisit:manual']"
              size="mini"
              type="primary"
              plain
              icon="el-icon-plus"
          ...
          <template #custom="data">
            ...
            <div v-else-if="data.props === 'operation'">
              <el-button
                v-has-permi="['confinement:returnVisitRecord:edit']"
...
import Submission from '@/components/ManualSubmission'
import distribution from '@/components/NoReturnVisitDistribution'
import moment from 'moment'
import { debounce } from '@/utils/throttleAndDebounce'
import { export_table_to_excel } from '@/utils/utils'; // 防抖
...
export default {
  ...
  created() {
    ...
    this.options.isReachCourtyard.options = this.dict.type.whether// 是否到院查询
  ...
  methods: {
    exportExcel() {
      ...
      this.$api.modules.netPower.returnVisitPlan(data).then(res => {
        ...
        export_table_to_excel('#out-table', '未到院顾客回访')
        setInterval(() => {
    ...
    returnVisit(numberId, isExecute, customerId) {
      // this.$router.push({ path: `/appointment/${id}` })
      // this.$router.push({ path: `/appointment/${id}`, query: { noHospital: this.noHospital } })//此处传递的id是numebrid 在另个页面接受后用来获取这一行的数据
      this.$router.push({ path: '/returnVisitDetial', query: { numberId: numberId, noHospital: this.noHospital, isExecute: isExecute, customerId: customerId }}) // 此处传递的id是numebrid 在另个页面接受后用来获取这一行的数据
    },
    // 表格所选删除
    deleteTableItem() {
      if (this.selectionData.length < 1) {
      ...
      this.$confirm('此操作将永久此数据, 是否继续?', '提示', {
        ...
        // const numberIds = this.selectionData[0].numberId
        this.$api.modules.netPower.remove(numberIds).then((res) => {
          if (res) {
    ...
    getUserList(deptId) {
      ...
      this.$api.user.getUserList(data).then(res => {
        if (res.code === 200) {
          ...
          res.data.forEach(item => {
            if (item.postCode === 'wdzxs') {
              this.options.memberCustomerService.options.push(item)
            }
          })
          res.data.forEach(item => {
            if (item.postCode === 'wdzxs') {
