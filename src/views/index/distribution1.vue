  ...
  <div>
    <div class="achievement">
      <div class="briefing_header">
        <div style="font-size: 14px;color: #666;margin-left: 20px;margin-top: 10px;font-weight: bold;">科室业绩及占比</div>
        <div>
...
export default {
  ...
  mounted() {
    ...
    disCom() {
      ...
      this.$api.deptId.companyObjectivesdeptPrmce(data).then(res => {
        if (res) {
            var newctive = [];
          newctive = res.data
            this.dataList=res.data
            newctive.forEach(item=>{
                item.value=item.performanceSum
                item.name=item.departmenName
            })
