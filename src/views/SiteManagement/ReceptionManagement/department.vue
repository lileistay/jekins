<template>
    ...
    <div>
        <slideSearch :options="options">
            ...
            <slot>
                <sg-table id="out-table" v-if="true" ref="table" :sg-ref="'multipleTable'" :pagination="pagination"
                    size="mini" selection border index="序号" highlight-current-row :table-data="tableData" :columns="columns"
                    :row-class-name="rowClassName" @size-change="sizeChange" @current-change="currentChange"
                    ...
                    <div slot="header" style="width: 100%;">
                        ...
                        <!-- <span style="margin-left: 10px; margin-right:10px;font-size: 14px"><i
                                class="color-block-success" />已到院</span>
                        <span style="font-size: 14px"><i class="color-block-default" />未到院</span> -->
                        <!-- <el-button v-hasPermi="['confinement:customerCmt:edit']" size="mini" type="info" plain
                            icon="el-icon-download" class="headerBarBtn" @click.stop.native="importCustomer">导入顾客信息
                        </el-button>
                        <el-button v-hasPermi="['confinement:customerCmt:edit']" size="mini" type="primary" plain
                            icon="el-icon-plus" class="headerBarBtn" @click.stop.native="handlecompile">修改顾客信息
                        </el-button>
                        <el-button v-has-permi="['Appointment:modification']" size="mini" type="success" plain
                            icon="el-icon-plus" class="headerBarBtn" @click.stop.native="appointmentAddition">添加预约
                        </el-button> -->
                        <!-- <el-button v-hasPermi="['sysManger:dept:Handmade']" size="mini" type="primary" plain
                            icon="el-icon-plus" class="headerBarBtn" @click.stop.native="openManualDialogs">分诊登记
                        </el-button> -->
                        <el-button v-hasPermi="['sysManger:dept:Handmade']" size="mini" type="primary" plain
                            icon="el-icon-plus" class="headerBarBtn" @click.stop.native="openManualDialog">手工上报
                        </el-button>
                        <!-- <el-button v-has-permi="['sysManger:dept:label']" size="mini" type="primary" plain
                            icon="el-icon-edit-outline" class="headerBarBtn" @click.stop="customerLabelEdit">客户标签
                        </el-button> -->
                    </div>
                    <template #custom="data">
                        ...
                        <div v-if="data.props == 'customSex'">
                            {{ data.custom.customSex == '0' ? '女' : data.custom.customSex == '1' ? '男' : '未知' }}
                        </div>
                        <div v-if="data.props == 'isSchedule'">
                            {{ data.custom.isSchedule == '0' ? '未排期' : '已排期' }}
                        </div>
                        <div v-if="data.props == 'isTherapy'">
                            {{ data.custom.isTherapy == '1' ? '未治疗' : '已治疗' }}
                        </div>

                        <div v-if="data.props == 'orderState'">
                            {{ getstatus(data.custom.orderState) }}
                        </div>
                            ...
                            <el-row>
                                ...
                                <el-col :span="12">
                                    <el-form-item label="客户性别">
                                        <el-select filterable v-model="manualForm.userInfo.customSex" class="full-width"
                                            placeholder="请选择性别">
                                ...
                                <el-col :span="12">
                                    <el-form-item label="经济能力">
                                        <el-select filterable v-model="manualForm.userInfo.economicCapacity"
                                            class="full-width" placeholder="请选择">
                                            <el-option v-for="dict in dict.type.economic_capacity" :key="dict.value"
                            ...
                            <el-row>
                                <el-col :span="12">
                                    <el-form-item label="标签类型">
                                        <el-select filterable v-model="manualForm.form.customerLabelTypeId"
                                            class="full-width" placeholder="请选择标签类型" @change="labelTypeSelection">
                                ...
                                <el-col :span="12">
                                    <el-form-item label="标签">
                                        <el-select filterable v-model="manualForm.form.GuestLabel" class="full-width"
                                            placeholder="请选择标签">
                                            <el-option
                                ...
                                <el-col :span="12">
                                    <el-form-item label="登记时间">
                                        <el-date-picker v-model="manualForm.form.registerTime" type="date"
                                            format="yyyy-MM-dd" value-format="yyyy-MM-dd" class="full-width"
                                            placeholder="选择日期" />
                                    </el-form-item>
                                </el-col>
                                <el-col :span="24">
                                    <el-form-item label="备注信息">
                                        <el-input v-model="manualForm.form.remark" placeholder="请输入备注信息" type="textarea"
                                            :rows="3" resize="none" class="full-width" />
                                    </el-form-item>
                                </el-col>
                                <el-col :span="12">
                                    <el-form-item>
        ...
        <manual-submission :show-manual-dialog="showManualDialog" :inner-dialog="innerDialog"
            :selection-data="selectionData" @closeInnerDialog="closeInnerDialog"
            @closeShowManualDialog="closeShowManualDialog" @selectAllpeople="selectAllpeople" :flagnew="true"
...
export default {
    ...
    methods: {
      ...
      this.$api.confinementRoom.skinTriageList({
        waitId:this.selectionData[0].waitId
      }).then(res=>{
        console.log(res,'dsadausdh')
      })
    ...
    //   this.$router.push({ name: 'TriageRegistrations' })
        ...
        getUserList(deptId) {
            ...
            this.$api.user.getUserList(data).then(res => {
                if (res) {
                    ...
                    // this.options.doctorId.options = this.doctor;
                    this.options.aestheticConsultant.options = this.esthetics;
                    this.options.createuserId.options = arr1;
                    this.options.visituserId.options=arr1;
                    this.options.customerRepresentative.options=this.customer;
                    // this.options.serviceAssistant.options = this.huli;
                    // this.options.visituserId.options = arr1
        ...
        getData: debounce(function (isSearch) {
            ...
            if(data.typeThreeIds){
                let typeThreeId=data.typeThreeIds;
                let typeTwoId=data.typeTwoIds;
                let filingId=data.filingIds;
                delete data.filingIds;
                delete data.typeTwoIds;
                delete data.typeThreeIds
                data.typeThreeId=typeThreeId;
                data.typeTwoId=typeTwoId;
                data.filingId=filingId
            }
            let date = data.endcreateTime;
            delete data.endcreateTime;
            data.endCreateTime = date;
            // data.endOrderTime = deta;
            ...
            this.$set(data, 'deptId', this.deptId)
            this.showDataDialog = true
            this.$api.confinementRoom.subsidiaryIddeclp(data).then(res => {
                if (res) {
                    this.tableData = res.rows
