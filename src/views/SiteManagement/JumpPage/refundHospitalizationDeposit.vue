  ...
  <el-scrollbar>
    <div class="management">
      <div style="padding: 6px 40px">
              ...
              <el-col :span="8">
                <el-form-item label="退款原因:" prop="reasonForRefund">
                  <el-select v-model="formInline.reasonForRefund" filterable>
                    <el-option v-for="(item,index) in dict.type.refund_reason" :key="index" :value="item.value" :label="item.label" />
                  </el-select>
                </el-form-item>
              </el-col>
            </el-row>
        ...
        <div class="formBody" style="min-height: 520px">
          ...
          >
            ...
            <el-table-column prop="rechargeAmount" label="总金额" />
            <el-table-column prop="remainingAmount" label="剩余金额" />
            <el-table-column fixed="right"  prop="refundAmount" label="当次退款金额" width="220">
              <template slot-scope="scope">
                <el-input v-model="scope.row.refundAmount" readonly onfocus="this.removeAttribute('readonly');" size="mini" @input="refundAmounts" />
              </template>
            </el-table-column>
          </el-table>
        </div>
      ...
      <div class="footer" style="position:relative">
        <div class="content" style="position:absolute;right:50px">
...
export default {
  name: 'RefundHospitalizationDeposit',
  dicts: ['refund_reason'],
  data() {
    return {
      submitLoading: false,
  ...
  created() {
    ...
    this.getHospitalizationDepositList()
  },
  methods: {
    /**
     * @author Nevin
     ...
     * @Description: 预约金列表
     */
    getHospitalizationDepositList() {
      this.$api.billingList.hospitalizationDepositList({
        ...
        if (res) {
          ...
          this.tableData.forEach(item=>{
            if (item.refundAmount==undefined){
              item.refundAmount=item.remainingAmount
            }
          })
          this.paidAmount=this.tableData[0].refundAmount
    ...
    refundAmounts() {
      this.tableData.forEach(item => {
        if (item.refundAmount > item.remainingAmount) {
          this.$message.error('退款金额不能大于剩余金额')
          item.refundAmount = item.remainingAmount
        }
        this.paidAmount = item.refundAmount
        this.$forceUpdate()
      })
    },
    /**
    ...
    submitRefund() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          this.$set(this.formInline, 'deptId', this.deptId) // 部门ID
          this.$set(this.formInline, 'customerStatus', this.data.customerStatus) // 部门ID
          this.$set(this.formInline, 'onlineUserId', this.data.onlineUserId) // 部门ID
          this.$set(this.formInline, 'id', this.data.id) // 部门ID
          this.$set(this.formInline, 'billType', '14') // 开单类型
          ...
          this.$set(this.formInline, 'amountPayable', this.tableData[0].refundAmount)
          this.$set(this.formInline, 'rechargeCardId', this.tableData[0].hospitalizationId)
          this.$set(this.formInline, 'aestheticConsultant', this.data.aestheticConsultant)
          this.$set(this.formInline, 'billingDepartment', this.data.billingDepartment)
          // this.$set(this.formInline, 'deptId', this.data.deptId)
          ...
          console.log('退款数据有什么数据',this.formInline)
