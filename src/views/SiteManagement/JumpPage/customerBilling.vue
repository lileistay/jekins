  ...
  <el-scrollbar>
    <div class="management" style="margin-bottom: 20px">
      <div style="padding: 6px 40px">
          ...
          <el-form ref="formInline" class="mentform" :rules="rules" :model="formInline" size="mini">
            <el-row :gutter="20">
              <el-col :span="6">
                ...
                >
                  <el-select
                    v-model="formInline.aestheticConsultant"
                    placeholder="请选择"
                    filterable
                    :filter-method="Consultant"
                  >
                    <el-option
              ...
              <el-col :span="6">
                ...
                <el-form-item
                  v-else-if="billing.departmentType === '6'"
                  label="美颜师:"
                  prop="doctorId"
                  style="margin-left: 80px;"
                >
                ...
                <el-form-item v-else label="医生:" prop="doctorId" style="margin-left: 80px;">
                  <el-select v-model="formInline.doctorId" filterable clearable :filter-method="mays" @clear="selectdoctor">
                    <el-option
                      v-for="item in doctor"
                      :key="item.value"
                      :label="item.label"
              ...
              <el-col v-if="billing.triageType !== 2" :span="6">
                <el-form-item label="推荐人:" prop="secondRecommender" style="margin-left: 80px;">
                  <el-select
                    v-model="formInline.secondRecommender"
                    clearable
                    filterable
        ...
        <div class="formBody" :style="{height:heightnew+'px',transition:'all 0.5s',cursor:'pointer',position:'relative'}">
          <span class="itemment" @click="loadmore(false)">商城订单</span>
          <div style="width:15px;height:15px;background:#f2f2f2;position:absolute;top:-10px;left:110px;display:flex;align-items:center;justify-content:center" @click="loadmore">
            <i class="el-icon-arrow-down" style="" v-if="heightnew==0">
          </i>
          <i class="el-icon-arrow-up"  v-if="heightnew>0">
          </i>
          </div>

           <div v-if="heightnew>0">
             <div style="height: 20px;display: flex;align-items: center;font-size: 14px">
               ...
               <span :style="{color:switchcolor,marginLeft:'20px'}">正常订单</span>
               <span>/</span>
               <span :style="{color:switchcolordefalute}">拼团/秒杀</span>
               <span style="margin-left: 10px">
                 正常总订单数：{{ Totalnoral }}
               </span>
               <span style="margin-left: 15px;">
                 秒杀/拼团总订单数：{{ listDataTotal }}
               </span>
             </div>

            <SgTable max-height="130"  :columns="columns" :table-data="listData" :pagination="pagination" @size-change="sizeChange" @current-change="currentChange">
              <template #custom="data">
            <div v-if="data.props === 'productType'">
             {{ data.custom.productType=='1'?"实物":"服务" }}
            </div>
            <div v-if="data.props === 'orderState'">
             {{getstatus(data.custom)}}
            </div>
...
export default {
  ...
  watch: {
    ...
    switchvalue(newV){
      if(newV){
        this.switchcolor=''
        this.switchcolordefalute='#409eff'
        this.getAllOrder()
      }else{
        this.getnorallist();
        this.switchcolor='#409eff'
        this.switchcolordefalute='';
  ...
  created() {
    ...
    this.billType = JSON.parse(sessionStorage.getItem('customerBillingitem')).billType
    // this.billing = JSON.parse(this.$route.query.billing)
    this.billing=JSON.parse(JSON.parse(sessionStorage.getItem('customerBillingitem')).billing)
  ...
  methods: {
    ...
    getstatus(item){
      // 0:待支付 1:待核销/待发货 2:已取消/已发货 3:已完成 4:退款中 5:已退款
      let str=''
    ...
    loadmore(flag){
      if(flag){
        let id= sessionStorage.getItem('BillId');
        let data={
          id:id,
          pageSize: this.pagination.pageSize,
          pageNum: this.pagination.pageNum,
        }
        this.$api.confinementRoom.pcOnlineOrderInfoList(data).then(res=>{
          this.pagination.total=res.total;
          this.listData=res.rows;
        })
      }else{
        if(this.heightnew==200){
          this.heightnew=0
        }else{
          let id= sessionStorage.getItem('BillId');
          let data={
    ...
    mays(val){
      if (val) {
        //取值
        let newlist = this.newdoctor.filter((item) => {
    ...
    getAllOrder(){
     ...
     this.$api.confinementRoom.pcPinTuanOrderList(data).then(res=>{
      if(res.code==200){
        this.pagination.total=res.total;
      ...
      this.$refs.formInline.validate((valid) => {
        if (valid) {
          ...
          if (this.billType !== undefined) {
            ...
            this.$delete(this.formInline, 'gradeName')
            this.$delete(this.formInline, 'intentionDescription')
            // this.$delete(this.formInline, 'isSecondary')  需要传值
            this.$delete(this.formInline, 'receptionStatus')
            this.$delete(this.formInline, 'receptionTime')
            this.$delete(this.formInline, 'secondOpeningUser')
            this.$delete(this.formInline, 'triageType')
