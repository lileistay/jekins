      ...
      <div style="padding: 6px 40px">
        ...
        <div class="formBody" style="min-height: 520px">
          ...
          >
            ...
            <el-table-column fixed="right" label="当次退款金额" width="220">
              <template slot-scope="scope">
                <el-input v-model="scope.row.refundAmount" readonly onfocus="this.removeAttribute('readonly');" size="mini" @input="refundAmount" />
              </template>
            </el-table-column>
          </el-table>
        </div>
...
export default {
  ...
  created() {
    ...
    this.getAdvancePaymentRecordList()
  },
  methods: {
    /**
     * @author Nevin
          ...
          this.tableData = res.rows
            this.tableData.forEach(item=>{
                if (item.refundAmount==undefined){
                  ...
                  this.paidAmount=item.refundAmount //给应付款金额赋值
                  this.advancePaymenId = item.advancePaymenId //给预约金退款金额赋值
                }
            })
        }
      })
    ...
    */
    refundAmount() {
      this.tableData.forEach(item => {
        ...
        this.advancePaymenId = item.advancePaymenId
        this.$forceUpdate()
      })
    },
    /**
    ...
    submitRefund() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          this.$set(this.formInline, 'onlineUserId', this.data.onlineUserId) // 线上客服
          this.$set(this.formInline, 'deptId', this.deptId) // 部门ID
          this.$set(this.formInline, 'id', this.data.id) //  顾客编号
          this.$set(this.formInline, 'billType', '12') // 开单类型
          this.$set(this.formInline, 'departmentId', this.data.departmentId) // 科室ID
          this.$set(this.formInline, 'amountPayable', this.paidAmount) // 应付金额
          ...
          this.$set(this.formInline, 'rechargeCardId', this.advancePaymenId)
          ...
          this.$set(this.formInline, 'doctorId', this.data.doctorId)
          ...
          //     commonNumber: item.advancePaymenId,
          ...
          //     totalAmount: item.rechargeAmount,
          //     deptId: item.deptId,
