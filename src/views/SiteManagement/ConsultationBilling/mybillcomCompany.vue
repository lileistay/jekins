<template>
  ...
  <div class="container">
      ...
      <slot>
        ...
        >
          ...
          <div slot="header" style="width: 100%;">
            ...
            <!--            </el-button>-->
            <el-button
              v-has-permi="['SiteManagement:refund']"
              type="danger"
              size="mini"
              plain
              style="float: right;"
            ...
            <el-button
              v-has-permi="['SiteManagement:submit']"
              type="primary"
              size="mini"
              plain
              style="float: right;"
            ...
            <el-button
              v-has-permi="['SiteManagement:withdraw']"
              type="danger"
              size="mini"
              plain
              style="float: right;"
            ...
            <el-button
              v-has-permi="['SiteManagement:subscription']"
              type="success"
              size="mini"
              plain
              style="float: right;"
    ...
    <vue-easy-print ref="printChargeSheet">
      <charge-sheet-print :print="print" :details="details" :total="total" :printcustomCard="printcustomCard" />
    </vue-easy-print>
...
<!--              <span style="margin-left: 20px;display: inline-block;width: 200px;">状态：{{ result.billingStatus==3?'已结账':result.billingStatus==2?'已下单':'已提交' }}</span>-->
        ...
        <div style="width: 95%;height: 10px;display: flex;line-height: 25px;font-size: 8px">
          ...
          <span style="display: inline-block;width: 50px;">性别:{{ customSex=='0'?'男':'女' }}</span>
  ...
  methods: {
    ...
    */
    expandChange(row) {
      ...
      this.printcustomCard=row[1].customCardNumber
    },
    /**
     * @author Nevin
     * @date 2022/6/6
      ...
      if (data.billType === '9' || data.billType === '11') {
        if (data.oldBillType === '6') {
          this.$api.billingList.orderDetailsList({
            orderNumber: data.orderNumber //修改传值及接口
          }).then(res => {
            if (res) {
    ...
    */
    printChargeSheet(data) {
      ...
      this.printcustomCard=data.customCardNumber;
    ...
    */
    ...
    getData: debounce(async function(isSearch) {
      ...
      // let createTime=newgetData(data.billTime[0])
      // let endcreateTime=newgetData(data.billTime[1])
      // delete data.billTime
      // data.billTime=createTime
      // data.endbillTime=endcreateTime
