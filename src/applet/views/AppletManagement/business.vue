<template>
    <div class="sdffdsjfkdsjf">
        <div style="position: absolute;right: 20px;top:0px;background-color: #fff;height: 50px;">
            <el-button v-has-permi="['client:companyLabel:add']" type="primary" size="mini" style="position: absolute;right:120px;top:145px;z-index:1000"
                @click="submit" :loading="submitcompant" v-if="!ifEdit">
                提交
            </el-button>
            <el-button v-has-permi="['client:companyBasicsInfo:edit']" type="primary" size="mini" style="position: absolute;right:120px;top:145px;z-index:1000"
                @click="submit" :loading="submitcompant" v-if="ifEdit">
                提交
            </el-button>
            <!-- ifEdit -->
        ...
        <div style="height:780px;width:100%;overflow-y:scroll" class="topscroll">
            <div class="basic_top">
                <div class="basic">
                ...
                <div style="display: flex;width: 100%;min-height: 30px;flex-wrap: wrap;margin-left: 80px;">
                    <div v-for="(items, indexs) in enterprise" :key="indexs + 100"
                        ...
                        <div v-show="items.flag" style="display: flex;align-items: center;">
                            <span style="margin-left: 20px;color: red;" v-show="items.name !== '企业规模'">*</span>
                            <span :style="{ marginLeft: items.name == '企业规模' ? '20px' : '' }">{{ items.name }}</span>
                            <img src="@/assets/images/wh.png" alt="" style="display: inline-block;margin-left: 5px;" :title="items.des" v-show="items.name !== '企业地址'&&items.name !== '来院路线'">
                            <el-input v-model="items.row" :style="{ width: '250px', marginLeft:items.name=='企业地址'||items.name=='来院路线'?'35px':'10px' }"
                                :maxlength="items.num" show-word-limit size="mini" :placeholder="items.placeholder"
                                v-if="items.name !== '企业面积' && items.name !== '企业规模'" clearable></el-input>
                            <div class="msx" v-else style="display:inline-block">
                                <el-input-number v-model="items.row" :min="0" label="描述文字" size="mini"
                                    style="margin-left:10px;width:250px" clearable></el-input-number>
                            </div>
                        ...
                        <div v-show="items.name == '营业时间'" style="margin-left: 30px;">
                            <span style="margin-left: 20px;color: red;">*</span>
                            <span>{{ items.name }}</span>
                            ...
                            <el-input style="width:120px"
                                v-model="remark" size="mini" clearable></el-input>
                            <!-- <span style="margin-left: 10px;color: red;">周一至周日</span> -->
                        </div>
                        <div v-show="items.name == '企业电话'" class="time" style="display:flex">
                            <span style="margin-left: 20px;color: red;">*</span>
                            <span>{{ items.name }}</span>
                            <el-input v-model="items.row" :style="{ width: '250px', marginLeft: '35px' }" maxlength="100"
                                  placeholder="请输入企业电话" :rows="4"  size="mini"></el-input>
                        ...
                        <div v-show="items.name == '企业微信'" class="time" style="display:flex">
                            ...
                            <span>{{ items.name }}</span>
                            <el-input v-model="items.row" :style="{ width: '250px', marginLeft: '35px' }" maxlength="100"
                                  placeholder="请输入企业微信" :rows="4"  size="mini"></el-input>
                            <p style="color:red">{{ items.des }}</p>
                        </div>
                        <div v-show="items.name == '企业简介'" class="time" style="display:flex">
                            <span style="margin-left: 20px;color: red;">*</span>
                            <span>{{ items.name }}</span>
                            <el-input v-model="items.row" :style="{ width: '500px', marginLeft: '35px' }" maxlength="100"
                                show-word-limit type="textarea" placeholder="请输入企业简介" :rows="4" clearable></el-input>
                            <p style="color:red">{{ items.des }}</p>
                        </div>
                ...
                <div v-for="(items, indexs) in enterprise" :key="indexs + 100" style="margin-left: 80px;">
                    <div v-show="items.flags"
                        style="width: 800px;min-height: 30px;display: flex;flex-wrap: wrap;font-size: 14px;align-items: center;margin-top: 10px;">
                        <span style="margin-left: 20px;color: red;">*</span>
                        <span>{{ items.name }}</span>
                        <el-tag v-for="(tag, index) in newtags" :key="index" closable :type="tag.type"
                            :style="{ marginLeft: '10px', marginTop: '5px' }" @close="closelabels(index)">
                ...
                <div style="margin-left: 80px;">
                    <el-form :model="ruleForm" ref="ruleForm" label-width="100px" class="demo-ruleForm">
                        ...
                        <!-- 企业图片 -->
                        <el-form-item :label="items.Name" prop="region" v-for="(items, indexs) in uploadlist.slice(0, 5)"
                            :key="indexs" :style="{ display: items.name == 'video' ? 'none' : '', position: 'relative' }">
                            <span :style="{ color: 'red', position: 'absolute', left: items.Name !== '品牌介绍图' ? '-78px' : '-90px' }"
                        ...
                        <el-form-item :label="items.Name" v-for="(items, indexs) in uploadlist.slice(5, 9)"
                            :key="indexs + 5">
                            <span :style="{ color: 'red', position: 'absolute', left: items.Name !== '执业许可证' ? '-78px' : '-90px' }"
                            ...
                            <div class="busic_img" :style="{ width: items.newwidth }">
                                ...
                                <div v-for="(item, index) in items.value" :key="index" class="busic_img_children"
                                    ...
                                    <div class="busic_imgs" @click="addlist(index, 'title_img' + items.name, indexs + 5)"
...
export default {
    ...
    watch: {
        deptId(newV) {
            this.$api.confinementRoom.getCompanyInfoByDeptIdedit({
                deptId: newV
            }).then(res => {
                if (res.data == undefined) {
                    ...
                    if (object['Img'].length == 0) {
                        ...
                        // console.log(object,'dsahfjdshfhsdf')
                    ...
                    this.enterprise[10].row = res.data.companyIntroduce;
    ...
    mounted() {
        ...
        this.$api.confinementRoom.companyLabel({ deptId: this.deptId }).then(res => {
        ...
        this.$api.confinementRoom.getCompanyInfoByDeptIdedit({
            ...
            if (res.data == undefined) {
                ...
                this.enterprise[12].row = res.data.companyIntroduce;
                this.newcompanyId = res.data.companyId
    ...
    methods: {
        ...
        submit() {
            ...
            let data = {
                ...
                companyIntroduce: this.enterprise[12].row,
    ...
    data() {
        return {
            ...
            uploadlist: [
                ...
                {
                    name: "invite", Name: "邀约到店", desc: '建议图片不超过2M，建议图片尺寸：750*1334',
                    value: [{
                        imgUrl: "",
                        border: '1px dashed #d9d9d9',
                        showflag: false,
                ...
                {
                    name: "photo", Name: "企业相册", desc: '建议图片不超过2MB，建议图片尺寸：248*164',
                    value: [{
                        imgUrl: "",
                        border: '1px dashed #d9d9d9',
                        showflag: false,
                ...
                {
                    name: "trading", Name: "营业执照", desc: '建议图片不超过2MB，建议图片尺寸：248*164',
                    value: [{
                        imgUrl: "",
                        border: '1px dashed #d9d9d9',
                        showflag: false,
                ...
                {
                    name: "operation", Name: "执业许可证", desc: '建议图片不超过2MB，建议图片尺寸：248*164',
                    value: [{
                        imgUrl: "",
                        border: '1px dashed #d9d9d9',
                        showflag: false,
                ...
                {
                    name: "advertising", Name: "广告审查", desc: '建议图片不超过2MB，建议图片尺寸：248*164',
                    value: [{
                        imgUrl: "",
                        border: '1px dashed #d9d9d9',
                        showflag: false,
